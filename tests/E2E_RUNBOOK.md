...
```
**Assert:** created 3 (the three real inboxes), skipped/normalized as expected; `reply-test@`
...
import json;print([c['id'] for c in s.a.query('contacts') if any(e['address']=='operator@example.com' for e in c['identities']['emails'])][0])")
...
Send a step-1 email to `reply-test@example.com` (repeat Step 3 with that address), then from that
