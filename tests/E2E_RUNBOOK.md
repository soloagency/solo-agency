...
- Access to the three test inboxes to act as the recipients:
  `operator@example.com`, `client@example.com`, `reply-test@example.com`.

Set once:
```bash
cd /path/to/outreachcrm
...
`test_fixture`-tagged suppression so `reply-test@` is not permanently blocked between runs.)
...
`reply-test@` is imported now (it becomes suppressed only in Step 5).
...
```bash
LEAD=$(python3 -c "import sys; sys.path.insert(0,'tools'); from crm_store import CrmStore; s=CrmStore('$CDIR'); print(next(c['id'] for c in s.a.query('contacts') if any(e['address']=='operator@example.com' for e in c['identities']['emails'])))")
echo "LEAD=$LEAD"
...
{"id":"draft_e2e_1","schema_version":1,"lead_id":"$LEAD","campaign_slug":"demo","step":1,
 "sendbox":"sb-a","to":"operator@example.com","subject":"Quick question about your listings",
...
```
**Assert:** the email arrives at `operator@example.com`; `sent_log.jsonl` has one row with a
...
From `operator@example.com`, **reply** to that email (keep the `Re:` subject). Then:
```bash
...
Send a step-1 email to `reply-test@example.com` (repeat Step 3 with that address). Each sent
email carries a `List-Unsubscribe: <mailto:{sendbox}+unsub-{token}@gmail.com>` header. In Phase 1
the **deterministic** opt-out is that mailto alias: from `reply-test@example.com`, **click the
...
```bash
python3 tools/gmail_client.py --client-dir "$CDIR" sync --sendbox sb-a
python3 tools/crm_store.py --client-dir "$CDIR" suppress check --email reply-test@example.com
```
...
```bash
# author a draft to reply-test@ and try to send
