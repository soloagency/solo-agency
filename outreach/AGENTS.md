...
Nothing sends without explicit human approval given in chat. The agent drafts, renders an operator-only Approval Report, and waits for chat approval (`approve all` / `approve 1-20, 35` / `reject N: reason` / `edit N: ...` / `hold N`). Default `approval_mode` is `manual_all` for every campaign and step, including bumps and assisted channels. After approval, who performs the send depends on the channel: the agent executes **email** (SMTP), **Messenger DMs** and **Facebook comments** (via the Local Collector's verified write capabilities); **SMS and Zalo** have no capability and stay assisted — drafted for the human to send by hand. See "Channel Execution Model" in `OUTREACHCRM_PLAYBOOK.md`. This is the agency's own outreach; the agent never messages or comments as, or on behalf of, a paying client.
