...
5. **Anchorless leads — resolution ladder first (DESIGN §9.1b): seed → profile → email → hooks.**
   ...
   **Resolving the OWNER is only half the job: the reel page has no contact info.** The collector
   ...
   URL. Walk these in order and stop at the first hit (`sample.business` shown as the example):

   | # | Open | Why |
   |---|---|---|
   | 1 | `facebook.com/sample.business` — the profile itself, and **expand "See more"** on the bio | very often printed right there; `sample.lead` only reveals `sample.lead@example.com` after See more |
