...
2. Follow the Daily Run order in playbooks/AUTOMATION_SCHEDULING.md. Load `{repo_root}/playbooks/TEAM_MODEL.md` and `{repo_root}/playbooks/LEAD_QUALIFICATION_RULE.md` before routed classification work, and use the canonical model-routing log schema from `{repo_root}/playbooks/07_STORAGE_SCHEMA_AND_HISTORY.md`.
...
7. Run the Daily Run in this order (playbooks/AUTOMATION_SCHEDULING.md has the full contract):
   ...
   c. Semantically triage replies through the mapped extractor tier file-in/file-out (Luna on Codex; at most one logged Terra retry per failed batch; otherwise stop `low_tier_subagent_unavailable`) into positive/question/objection/negative/remove_intent, then run tool crm-store apply-rules (deterministic): reply_positive -> create deal + "reply within 4h" task + freeze sequence; negative/remove_intent -> suppress + freeze; SLA sweep -> nudge tasks. Every stage change carries an evidence activity.
   ...
   i. Compile the Today View and regenerate the kanban (tool render-report, operator-only).
   j. Produce the daily ops report + refresh the operator-only Approval Report + INTERNAL_REPORT (operator-only). On the weekly day, also produce the weekly client report (`tool crm-store weekly-report`), and on the first run of a new month the prior month's monthly client report (`tool crm-store monthly-report --month <prior YYYY-MM>`) — both through the Client-Blind Scrub Gate.
...
If there is a new commit, compare root instructions, playbooks, tools (tool crm-store/tool gmail/tool import-leads/tool verify-email and the storage adapter), tracker/worker.js, storage schema/schema_version, setup scripts, templates, and automation contracts.
...
- `{repo_root}/playbooks/TEAM_MODEL.md`, `{repo_root}/playbooks/LEAD_QUALIFICATION_RULE.md`, and the model-routing schema in `{repo_root}/playbooks/07_STORAGE_SCHEMA_AND_HISTORY.md` before routed classification
...
1. Produce the operator reports (Approval Report, Today View, daily ops, INTERNAL_REPORT) via `tool crm-store` (`approval-report`/`today-view`/`kanban`) and, on the weekly day, the scrubbed weekly client report via `tool crm-store weekly-report --client-name "…"` — and on the first run of a new month, the prior month's monthly client report via `tool crm-store monthly-report --client-name "…" --month <prior YYYY-MM>` (both render through `tool render-report --client-facing --fail-on-scrub`; a blind-term hit blocks the ship).
2. Send the notification in one composed step: `tool provider --config <client provider_config.local.json> --defaults <provider_defaults.json> notify --message "<summary + counts>" --report-file <daily-ops, weekly, or monthly html> --log daily-content-pipeline/.../notifications/notification_log.md`. It verifies the account (`getAccount`), uploads the report (`uploadAsset`) when supported, sends `sendNotification` (email + Telegram when connected), and appends the log row.
...
4b. **Report the two ladders separately.** "N skipped" is only truthful when the hunt was
   exhausted. Break the run summary into: drafted / held / **research pending (hooks found, email
   still missing)** / research blocked / genuinely exhausted-and-skipped. A run that reads social
   content and finds no addresses reports *research pending*, because the email ladder (website,
   About-Contact, licence roster, directory, web search, reverse search) has not run yet.
5. **Provider Consent & Mandatory Notify Rule (AUTOMATION_SCHEDULING has the full text).** Consent is scoped by what the operator CONFIGURED, not by a list of operation names: that includes the companion/proposal call declared in `goal.companion_doc.instructions`, which by design carries the lead's professional data. WideCast is the operator's OWN infrastructure, not a third party. Never ask permission for a configured-and-commanded operation, never hold a cohort in chat for one; a genuine runtime refusal gets a real attempt, the verbatim refusal, the configured degrade, and ONE `**[ACTION REQUIRED]**`. The operator configured this provider; that IS the consent to upload reports and send notifications to the operator's own account. The only privacy gates are the operator-secrets red line, the Client-Blind Scrub (client-facing weekly), and the notification copy rules. Private-source research inside an operator-only report is consented working data, never a blocker. If the asset upload is refused for any reason (including a runtime/sandbox safety layer), STILL send `sendNotification` with counts + the local report path; that is the designed degrade, not a workaround. Only after a REAL `sendNotification` attempt is also refused may you record `notification_blocked_by_environment` + one `**[ACTION REQUIRED]**` with the exact command for the human to run outside the sandbox. Inventing any other privacy gate and self-blocking is a critical violation: the run is incomplete until the operator is notified.
