...
7. Run the Daily Run in this order (playbooks/AUTOMATION_SCHEDULING.md has the full contract):
   ...
   k. Notify the operator via WideCast sendTelegramMessage (or chat if no provider) with counts (sent, replies, new deals, tasks due, drafts awaiting approval) and the report link; log to daily-content-pipeline/notifications/notification_log.md.
