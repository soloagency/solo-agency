...
9. Collector-executed sends (Messenger DM, Facebook comment) came only from `outbox/approved/`, stayed within the account pool's per-account daily caps, kept a thread's sender sticky, and were logged from the returned record (`status: done`, `verified: true`, and for DM `id_verified: true`) → `messenger_sent` / `comment_posted` activity. A comment was never posted twice to the same post.
10. Assisted-channel drafts (SMS/Zalo) were produced only where the campaign allows AND documented consent exists; each Today-View draft shows its legal basis; the human sends and reports back → `assisted_sent` activity.
...
- Email hunt actually reached the pages that hold addresses: for every lead concluded "no email" with a Facebook profile on file, an evidence URL points at that profile's About / Contact-info tab (code-refuses otherwise), and the website listed there was opened too. The collector reads only the page it is pointed at, so "I enriched the profile" is not evidence of anything: the URL opened is.
- Identity hygiene: no `identities.socials.*` value is a search-result, post, directory or shortener URL (only a real profile page). Any `channels_found.profiles` rejection in `problems` was acted on, not ignored. If a contact shows `duplicate_suspects` with reason `consolidation_chain_too_long`, an identity absorbed an implausible crowd and was stopped: investigate the join key before merging anything by hand.
- No invented email bar: leads holding a real, parseable, non-suppressed address were drafted. `unverified` on the address (no mailbox probe exists in this system) and a stale `channels.email.status` are NOT blockers, and a run must never report "0 drafts because emails are unverified" while sitting on real addresses.
- Two-ladder honesty: hook discovery and email discovery are reported SEPARATELY, with their own counts. A lead whose dossier has hooks but no address is **research pending** (`enrich status` → `email_discovery`), never "skipped" — "skipped" asserts the hunt was exhausted. A social-only pass is not an email hunt: verify that any `mark_email_not_found` was accepted (a facebook-only dossier is refused in code) and that the run left the social pass (website / About-Contact / licence roster / directory / web search / reverse search) before concluding no address exists.
- Personalization honesty (code-enforced by `draft write`; report the counts): no step-1 draft went out generic to a contact that HAD dated hooks (`hooks_available_but_unused`); every declared hook is actually woven into the subject/body, not just listed in `hooks_used` (`hook_not_woven`); no undated `website_update` was treated as proof-of-life; leads refused for lack of evidence are reported as HELD (`draft_held_no_evidence`) with a count, never silently dropped, and never re-drafted as filler to use up quota.
...
- **Consent scope is what the operator CONFIGURED, not a named list:** verify no lead was held, and no cohort stalled, waiting for permission on an operation the operator's own config already declares — provider ops (`uploadAsset`, `sendNotification`) AND the companion/proposal call in `goal.companion_doc.instructions`. A consent question asked for a configured-and-commanded operation is a critical violation; a genuine RUNTIME refusal is different and must show a real attempt, the verbatim refusal, the configured degrade (`on_fail` / `local_path_only`), every other lead still processed, and ONE `**[ACTION REQUIRED]**` — never a consent quiz and never a per-lead re-ask.
- **Consent + mandatory-attempt honesty (Provider Consent & Mandatory Notify Rule):** the configured provider is the operator's own delivery channel; "the report contains private-source research" / "external disclosure" / "indirect workaround" are NOT valid blockers (the only privacy gates are the operator-secrets red line, the Client-Blind Scrub on the client-facing weekly, and the notification copy rules). Verify a REAL `sendNotification` attempt was made this run (a `sent`/`failed`/`notification_blocked_by_environment` row with tool evidence, not a self-blocked `blocked_not_sent`); a blocked upload with no notification attempt is a critical violation. If the environment refused, verify the log holds the verbatim refusal AND the human got the exact outside-sandbox command in an `**[ACTION REQUIRED]**`.
...
8. **Collector-executed channels:** for `messenger` / `comment` campaigns, execute the already-approved drafts through the Local Collector within the account-pool caps → `messenger_sent` / `comment_posted` activity. For `friend_harvest` campaigns (Stage 16) the agent NEVER enqueues collector jobs — the daemon walks and enriches within budgets; the agent only judges `harvest pending` records against the goal and records `harvest decide` (kept → CRM contact, matched-before-created; rejected/enrich_failed remembered client-wide). An agent-issued `fb.profile.friends`/`fb.profile.enrich` job for a harvest campaign is a violation (it doubles the account footprint the daemon bounds), as is a `kept` without a `--lead-id`, and a `kept` friend whose enrich record listed a website but no email that was decided without the Stage 4 website/off-platform email hop (a reachable lead left as a name). For `zillow_harvest` campaigns (Stage 17) the daemon writes the CRM itself; an agent-issued `zillow.agents.list`/`zillow.profile.enrich` job, or an agent "deciding" Zillow records, is the same violation — the run only reports `harvest status` and raises the bot-check ACTION REQUIRED when `zillow.blocked` names an account. **Assisted channels:** draft SMS/Zalo for no-email contacts where the campaign allows + consent exists → Today View copy buttons; human sends, reports back → activity.
...
11. **Notify** via WideCast `sendNotification` (email + Telegram when connected): counts + report link → `notification_log.md`.
...
7. Assisted-channel drafts (SMS/Zalo) were produced only with consent and a shown legal basis; the human sent and reported back → `assisted_sent` activity. Collector-executed channels (Messenger/comment) sent only approved drafts, within caps, and logged the returned verification.
...
- [ ] Social-FIRST enrichment: for every lead with a Facebook profile on file, did I READ it through the collector with ONE `fb.profile.enrich` job on the ROOT url (posts+reels and About in one record) and analyze 3–5 recent `posts[]` items, rather than saving the URL and stopping at the website? Did I refuse to treat a `website_update` line as proof-of-life?
- [ ] One profile, one load: did any lead get a second collector job on the same profile (`fb.profile.contacts` / `fb.profile.header` / `fb.profile.videos` after an enrich pass, or an enrich pass submitted at `/about` that came back with `posts: []`)? Each is a wasted page load and extra account footprint — a defect, not a retry.
...
- Notification delivery via WideCast email+Telegram.
