...
8. **Collector-executed channels:** for `messenger` / `comment` campaigns, execute the already-approved drafts through the Local Collector within the account-pool caps → `messenger_sent` / `comment_posted` activity. **Assisted channels:** draft SMS/Zalo for no-email contacts where the campaign allows + consent exists → Today View copy buttons; human sends, reports back → activity.
