...
layer, not a replacement. Operators see the live picture on `/ui/{client}/sent`. The same tick also pulls open/click/unsub tracking events from WideCast (when the client's `provider_config.local.json` has a `tracking` block: just `enabled` + `base_url` — the bridge fetches its per-tenant signing secret with the wc_live_ key it already has, so the operator pastes no new secret) into `outreach/tracking/events.jsonl` — the Sent page shows opened/clicked, and a one-click unsub becomes a real suppression. No tracking block = reply-only, everything else unchanged.
