...
**Set the expectation: rows ≠ people.** A reel-heavy list often carries several content links of the SAME person, which import cannot know yet — each row imports as its own record, and enrichment's resolution ladder consolidates them the moment two fragments resolve to one profile/email (auto-merge with full union; DESIGN §9.1b consolidation-on-discovery). A `matched` outcome at import already unions the row's NEW data (another seed, a second phone) into the existing contact — never dropped. So report imports as "N rows → M records (K matched into existing)", and tell the operator the record count may shrink further during enrichment as fragments collapse into people — that is dedupe working, with nothing lost.
...
Import stores what it reads: `name`, `identities.emails/phones/socials/website`, and `company`/`city`/`state` into `contact.custom_fields`. **It also keeps every OTHER column of the source list verbatim in `custom_fields`** (and in the list audit row's `normalized.extra`): NPN, licence number/type, years in practice, agency/brokerage, MLS or DRE id, business address, operator notes. Nothing a rich list carries is thrown away, because those columns are research clues — Stage 4 reads them first to resolve a profile/email instead of searching blind (`04_VERIFY_ENRICH.md` §4b). A mapped field always wins its canonical slot on a name clash. Storing is not using: personal attributes that ride along (age, gender, home address/phone) are kept for provenance only, never used for segmentation and never in copy (`do_not_mention`). It does not create Account records or resolve accounts — company clustering and account linkage are later-stage CRM work (Stage 13). Imported phones are stored E.164-normalized with `type: cell`.
