...
9. Every client-specific automation task name must begin with the client name, e.g. `Max Output - Startup Founders Intro Daily Run`.
...
5. If the newest version still leaves setup blocked, report it on the first rung this install can reach (`AGENTS.md`, the reporting ladder): **WideCast** `reportError` when the client's `provider_config.local.json` carries a key (`module` starting `solo-agency/outreach`), the **Solo Agency Facebook support group** when the install is keyless or that call is unavailable (`playbooks/TEAM_MODEL.md`, Support requests — the human approves the exact text first), a **GitHub issue** (title prefixed `outreach:`) only when the runtime ALREADY has an authorized identity, and otherwise a ready-to-post draft under `daily-content-pipeline/automation/issues/`. Never ask the human for a GitHub account.
...
For each configured client, Setup Flow must leave these current (all CRM writes via `tool crm-store`). Write CRM records (pipelines, segments, custom-field definitions, contacts) through `tool crm-store` — it exists (Phase 1). The Client Intelligence Profile is a `.md` file written directly. A workspace updated from an older Phase-0 install must run `<bridge> tool crm-store --client-dir <DIR> validate --rebuild-index` once per client before use (DESIGN §22 R3).
...
For sendbox connection, the setup handoff must present the connection steps in an `**[ACTION REQUIRED]**` block, not just a status line. `tool gmail` exists (Phase 1): the handoff includes the App Password creation steps (Google Account → Security → 2-Step Verification → App passwords) plus the exact command — set `OUTREACHCRM_APP_PASSWORD` in the human's shell, then `<bridge> tool gmail --client-dir <DIR> auth --sendbox <slug> --email <you@gmail.com>`, which verifies SMTP+IMAP and writes the box as `status: healthy`.
