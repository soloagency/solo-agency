...
- **All CRM mutations go through `tool crm-store`.** Writing directly to any file under `clients/{slug}/crm/` (contacts, accounts, deals, activities, tasks, pipelines, suppression) is a critical violation of the inherited "no one-off scripts" rule. Reading raw JSON is allowed only for debugging.
...
- **Guessed email is code-enforced.** Guessed/unverified addresses require an explicit guessed-approval flag on the draft plus a daily guessed-send cap, both enforced in `tool gmail send`. First guessed-pattern hard bounce at a domain suppresses all other guessed addresses at that domain.
...
- running the bridge tool subcommands (`tool crm-store`, `tool gmail`, `tool import-leads`, `tool verify-email`, `tool render-report`, `tool provider` — all inside the single collector bridge binary; no Python required) and, when tracking is enabled, a Cloudflare Worker under `trk.{domain}`.

### Binary-First Tool Invocation

Every tool lives INSIDE the collector bridge binary as a subcommand (the original Python implementations were retired 2026-07-19 after golden cross-validation and a live sendbox run — the binary is the ONLY implementation):

| Tool (legacy name) | Bridge subcommand |
|---|---|
| `tool crm-store` | `<bridge> tool crm-store` |
| `tool gmail` | `<bridge> tool gmail` |
| `tool import-leads` | `<bridge> tool import-leads` |
| `tool verify-email` | `<bridge> tool verify-email` |
| `tool render-report` | `<bridge> tool render-report` |
| `tool provider` | `<bridge> tool provider` |
...
1. The installed bridge binary subcommand — the only path. Probe once per session: `<bridge> tool crm-store` must print usage (exit 2), not an unknown-command error.
2. An older binary that fails the probe means the install needs the collector setup re-run (it downloads the current binary); there is NO Python fallback — the `.py` tools no longer exist in the repo.

**`<bridge>` = ONLY the installed runtime binary at `{setup-root}/solo-agency-local-collector/bin/collector-bridge-{os}-{arch}`, matched to THIS machine's OS/arch.** NEVER run a binary from the source tree `solo-agency/solo-agency-collector/bridge-go/` — that directory holds Go SOURCE, and any executable found there is a stray dev/CI build (often for a different OS, e.g. a Linux ELF that macOS cannot launch) that is NOT the install. If the installed binary is missing for this machine's platform, re-run the collector setup (it downloads the matching binary); do not build from source or run a cross-OS binary.

**Two roles, one binary — do not conflate them.** (a) As a running SERVER it is the Local Collector on `127.0.0.1:17321`; you never call that server directly from a sandbox — you command it by writing job files and check it by reading status files (Sandbox Localhost Rule). (b) As a one-shot CLI you EXECUTE it as a subprocess (`<bridge> tool crm-store …`, `<bridge> tool gmail …`) to read/mutate lead data and send email. The daily campaign run needs role (b) — running the tool subcommands on the lead data under `daily-content-pipeline/clients/{slug}/…/outreach/crm/` — even when no collector/discovery is involved; that is normal subprocess execution, not a localhost call. Reading raw JSON for a quick look is fine, but every CRM MUTATION and every send goes through the tool subcommand, never a hand-written file.

Windows runs fully through the binary (the storage layer uses native file locks, not fcntl). Never tell a human to install Python — nothing in Solo Agency needs it.
...
- **All CRM mutations go through `tool crm-store`.** Never hand-edit or one-off-script a file under `clients/{slug}/crm/`. Every record carries `schema_version`, `id`, `created_at`, `updated_at`; the adapter applies its upgrade registry on read. Raw reads for debugging only.
...
- **Guessed email is code-enforced.** A guessed/unverified address sends only with an explicit guessed-approval flag on the draft and within a daily guessed-send cap, enforced in `tool gmail send`. Route guessed/unverified through a third-party verification API; exclude/limit `catch_all`; kill all guessed sends at a domain after its first guessed hard bounce; report the guessed cohort's bounce rate separately.
...
- Lists arrive as CSV/TXT/XLSX and are imported through `tool import-leads` into `crm/contacts/{lead_id}.json`. `lead_id` is a **ULID minted at import** — not a hash of email, because **email is NOT required**. A contact may enter with only a name and a social URL.
...
- **Dedupe/merge is deterministic.** Auto-merge on exact email / E.164 phone / canonical social-URL match — at import (matched rows union their new data in) and at enrich time (consolidation-on-discovery: many reels resolving to one profile fold into one record, full union, nothing loaded is lost). The losing record becomes a permanent tombstone (`merge.status = merged`, `merged_into`), never deleted; identities, channel statuses, and suppression are **unioned** into the survivor. Every `lead_id` lookup path calls `resolve(lead_id)` to follow merge chains. Records that share an identity but CONFLICT (different names / disjoint emails) are never auto-merged: both carry `duplicate_suspects[]` and are excluded from every campaign queue until the operator merges (`contact merge`) or clears (`contact unsuspect`).
...
- **Guessed email.** MX alone is near-meaningless on catch-all domains. Route guessed/unverified addresses through a third-party verification API (called by the bridge's `tool verify-email`); `catch_all` → excluded from the guessed quota or capped ~2%; per-domain kill switch on the first guessed hard bounce. `guessed_only` sends are enforced in `tool gmail send` (guessed-approval flag + daily cap), never only in prose; the guessed cohort's bounce rate is reported separately.
...
Load Stage 8 (`08_SEND_ENGINE_PROTOCOL.md`) before any send. Sending is `tool gmail send` per draft, in code — do not trust playbook prose for the checks.
...
- **Guessed email policy.** As in §3C: third-party verification, `catch_all` handling, per-domain kill switch, code-enforced guessed-approval flag + daily cap in `tool gmail send`, separate bounce-rate reporting. A guessed address never sends silently.
...
## 5. Storage & CRM Mutation Discipline (`tool crm-store`)
...
- The store is backed by a pluggable adapter (the storage layer inside the bridge binary, `store.go`): JSON files are the default backend, Postgres comes later and must pass the same parametrized contract tests. Interface includes `get/put/update/delete/query/append/read_log/find_by_identity/reserve`. `query` uses a flat-field `Cond` DSL (`= != < > contains in`); identity lookups do NOT use it — they use `find_by_identity` over the maintained unique reverse index `contact_identities`.
