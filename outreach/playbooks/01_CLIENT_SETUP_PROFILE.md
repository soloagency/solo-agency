...
- If the human asks to send, enrich-and-send, run a campaign, "email them now", or "just send the first batch" while this stage is being used for Setup Flow, do not send. Refuse with the exact wording in the Setup Flow Send Refusal section, finish or resync the client-specific automation task, and dispatch that task (the run happens in the task's own session, never in the setup chat) and say so, falling back to naming the task for the human only when the runtime cannot start it.
...
- If the human asks to send, enrich, run a campaign, generate drafts, or "start emailing" while still in Setup Flow, verify/resync the automation task instead and dispatch that task (the run happens in the task's own session, never in the setup chat) and say so, falling back to naming the task for the human only when the runtime cannot start it. Do not ask whether to send now, do not load `playbooks/SCHEDULED_RUN_ENTRYPOINT.md` in the setup chat, and do not perform enrichment, drafting, sending, tracking, or notification.
