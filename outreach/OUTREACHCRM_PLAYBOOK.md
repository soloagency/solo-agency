...
- **Every command or path you hand the human to run or open must be ABSOLUTE — the full path from the real workspace root, never relative. A non-technical user does not know which directory to run a command from.** This covers the tool/script path (e.g. `tool gmail`), the `--client-dir` value, and any file paths. Resolve the actual absolute workspace path and substitute it; do not hand out relative forms like `--client-dir daily-content-pipeline/...` — resolve absolute paths first.
...
OutreachCRM uses WideCast only to notify the operator (email + Telegram in one `sendNotification` call (required `subject` + `message`; Telegram added automatically when connected), optional `uploadAsset` for the report link). It is NOT used for producing or publishing content. Provider setup is client-scoped: read the client's `integrations/providers/provider_config.local.json`, verify identity via the client's API key, and check the client's OpenAPI capabilities before claiming notification is available. Before asking the human for a WideCast key, read existing config first (Stage-1 WideCast key bootstrap): check OutreachCRM's own provider config, then the SAME client's sibling Solo Agency content-pipeline provider config one level above the `outreach/` subtree; if a client-scoped key already exists and verifies there, reuse it (the operator's own account, already connected for this client) instead of re-asking. Only ask when no client-scoped key exists. Reusing the same client's sibling config is allowed; adopting a global MCP/native account as this client's connection stays forbidden. Save the key only as `api_key_env` or `api_key_local` — never a field named `api_key` (the OpenAPI helper ignores that field). Notification is optional: with no provider configured, surface report links in chat and log the blocker. Use `https://widecast.ai/app/dashboard` as the server; treat `https://api.widecast.ai` as a disabled host. Preferred operation: `sendNotification` (`POST /v1/notification/send`, required `subject` + `message`) - one call delivers email always and Telegram when connected; read the response `delivery`/per-channel statuses and log a 502 partial failure per channel; use legacy `sendTelegramMessage` only when the refreshed OpenAPI cache lacks the new operation, and re-run provider discovery per client after updating to this version. Notification copy is retention-first: a concrete, urgency-first subject with real counts (drafts awaiting approval, replies needing triage - never a mechanical status line) and a body that ends with next-action guidance naming the exact reply to give the agent.

## Two-Lane Reporting Rule

Operator-only reports (Approval Report, Today View, daily ops, `INTERNAL_REPORT`) carry full internal detail and are NOT scrubbed. The **weekly client report** is the ONLY client-facing deliverable; it must pass the Client-Blind Scrub Gate (no OutreachCRM, WideCast, sendbox, crm_store, tracker domains, API keys, Telegram, automation, or debug details — see `tool render-report` `CLIENT_BLIND_TERMS`). Render every report with `tool render-report`; do not write one-off report scripts. Use `--client-facing --fail-on-scrub` only for the weekly client report.
...
| 2 | `playbooks/02_SENDBOX_SETUP.md` | Connecting or checking a sendbox (App Password / OAuth), warmup, quota. (tool: `tool gmail`) |
| 3 | `playbooks/03_IMPORT_LIST.md` | Importing a CSV/TXT/XLSX list; mapping, dedupe, suppression checks. (tools: `tool import-leads`, `tool verify-email`) |
| 4 | `playbooks/04_VERIFY_ENRICH.md` + skill `email-verify-enrich` | Before any enrichment (verify still-active, gather evidenced hooks, distill writing brief). (tool: `tool crm-store enrich`) |
| 5 | `playbooks/05_CAMPAIGN_MANAGEMENT.md` | Create/edit a campaign, define a segment, populate the enrich queue. (tool: `tool crm-store` campaign/segment/queue) |
| 6 | `playbooks/06_EMAIL_WRITING_STANDARD.md` + skill `email-writing` | Before drafting any email; goal_type → structure. (tool: `tool crm-store draft`) |
...
| 8 | `playbooks/08_SEND_ENGINE_PROTOCOL.md` | Before any send: pre-send re-check chain, rotation, tracking, threading. (tool: `tool gmail`; mutations via `tool crm-store`) |
...
| 17 | `playbooks/17_ZILLOW_HARVEST.md` | Leads From Zillow (`zillow_harvest` channel): daemon walks the agent directory location × keyword × page and writes email/phone profiles into the CRM itself; the agent only reports and handles the bot-check ACTION REQUIRED. Load whenever such a campaign exists. |
...
1. Update the Client Intelligence Profile and relevant CRM config via `tool crm-store`.
...
Reporting: the human needs neither git nor a GitHub account — report it on the first rung this install can reach (`AGENTS.md`, the reporting ladder): **WideCast** `reportError` when the client's `provider_config.local.json` carries a key (`module` starting `solo-agency/outreach`), the **Solo Agency Facebook support group** when the install is keyless or that call is unavailable (`playbooks/TEAM_MODEL.md`, Support requests — the human approves the exact text first), a **GitHub issue** (title prefixed `outreach:`) only when the runtime ALREADY has an authorized identity, and otherwise a ready-to-post draft under `daily-content-pipeline/automation/issues/`. Never ask the human for a GitHub account. `gh issue create` is used only when `gh auth status` passes or `GITHUB_TOKEN` / `GH_TOKEN` / `OUTREACHCRM_GITHUB_ISSUE_TOKEN` / a maintainer bot is configured. Track every issue in `daily-content-pipeline/automation/github_issues.md`. Never include secrets, API keys, tokens, sendbox credentials, client-confidential data, or contact PII in an issue.
...
- All CRM mutations go through `tool crm-store`. Never write CRM state directly to a file.
...
- Render all reports with `tool render-report`; no one-off report scripts.
...
- The Client Intelligence Profile saved as its `.md` file at the correct path; pipeline, custom fields, segments, and contacts saved via `tool crm-store` (it exists, Phase 1 — a direct `crm/` write is a critical violation). Sending identity (from-name, signature, physical address, unsubscribe method) is recorded on the profile. A workspace carried over from an older Phase-0 install runs `tool crm-store validate --rebuild-index` once (DESIGN §22 R3).
...
- The diff scope covered playbooks, tools (`tool crm-store`/`tool gmail`/`tool import-leads`/`tool verify-email`), `tracker/worker.js` + its deploy step, the storage adapter/schema, and automation contracts.
...
- CRM rules were applied deterministically via `tool crm-store apply-rules`; stage changes carry evidence activities.
...
- If about to move a CRM object, create a deal, or write any CRM state, do it through `tool crm-store`, never by editing a file directly.
...
- Did every CRM mutation go through `tool crm-store`, and did I stay within the pinned client's workspace?
