...
1. `operator@example.com` — happy path: receive→(open)→click→reply positive→deal+task+follow-up draft.
2. `client@example.com` — own domain: MX check; no open/reply → bump due at day 4 (via injectable clock).
3. `reply-test@example.com` — reply "unsubscribe" → suppression + close tasks + never re-drafted.
