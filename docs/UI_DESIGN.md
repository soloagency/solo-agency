...
| `/ui/{client_slug}/sent` | Sent history: every sent email (newest first) joined read-only from `sent_log` + reply activities + pulled tracking events: recipient, campaign, step (cold/bump N), sendbox, sent time, and one status pill (replied > clicked > opened > sent). Stat tiles (total / replied / reply rate) + client-side filter. Rows click through to the contact | T1 |
