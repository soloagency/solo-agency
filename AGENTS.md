...
Limits marked "per client" apply inside each client workspace; clients and sends/day count the whole install. A capability's `feature` (catalog field) must appear in the token's `features` for it to run. Rollout: the bridge flag `--entitlement-mode` is `log` in this release (would-be refusals go to `daily-content-pipeline/collector/logs/entitlement.jsonl`, nothing is refused) and becomes `enforce` later; never promise the human a limit that `mode: log` is not applying.
