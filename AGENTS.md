...
## Free and Pro plans (entitlement)

Solo Agency runs on two plans, and the CODE enforces them — playbooks only read the result and speak to the human about it.

- **Free (keyless, forever):** 1 client, 1 watched private source, unlimited CRM contacts/deals/tasks, the morning brief, 1 outreach campaign, 20 sends/day across all sendboxes, public reports, discovery, healthcheck. An install with no key is Free and needs no network call for that.
- **Pro:** every private source the human is a member of, multi-client, the full outreach engine (unlimited campaigns), enrich/dossiers, lead harvest, write actions (react / comment / group post / Messenger DM), Zillow, priority adapter fixes. WideCast credits stay a separate add-on on the same account.
- **One key:** the client's WideCast API key from Step 7 is also the Solo Agency license. The bridge exchanges it for a signed entitlement (`GET /v1/solo/entitlement`), caches the raw token at `daily-content-pipeline/collector/inbox/entitlement.json`, re-verifies the signature on every read, and exposes the result read-only at `GET /status` → `entitlement` (`tier`, `features`, `limits`, `source`, `stale`, `mode`, `upgrade_url`) and in `bridge_health.json`. `<bridge> tool entitlement status|refresh --pipeline daily-content-pipeline` shows or forces it. Offline: the last verified plan keeps working for 14 days, then the install quietly runs as Free until the next successful check — never an error, never a locked CRM.
- **Rollout mode:** the bridge flag `--entitlement-mode` is `log` in this release (every would-be refusal is written to `daily-content-pipeline/collector/logs/entitlement.jsonl`, nothing is refused) and becomes `enforce` in a later release. Treat `mode` as informational; never promise the human a limit that `mode: log` is not applying.

**Upsell rule (applies to every brain, every flow).** The moment the human asks for a Pro feature, or a run reaches one, while `entitlement.tier` is `free` — or the bridge answers with `solo_feature_not_in_tier`, `solo_tier_limit_reached` or `solo_send_quota_exhausted`, or the extension reports `solo_entitlement_required` — finish the Free part of the work, then offer the upgrade IMMEDIATELY in one `**[ACTION REQUIRED]**` block: name the feature and the limit in plain words, say what Free keeps working, and give the exact path. Keyless human: register at `https://widecast.ai/#setup` (the page opens straight on the API-key screen; a new visitor is asked to log in or sign up), follow the standard key steps below, paste only the API key back, and the bridge picks the plan up within a minute (`<bridge> tool entitlement refresh --pipeline daily-content-pipeline` forces it). Human who already has a key: upgrade the plan on that same page. Value-first, no pressure language, once per session per feature, never inside client-facing deliverables. Do not work around a limit: never edit `entitlement.json` (its signature is verified), never hand-create a second client folder, never split one campaign into several to dodge the cap. If the human says they upgraded but the bridge still says `free`, run `tool entitlement refresh`, show `last_error`, and escalate per the blocker rules.
