...
MODE="all"
DEFAULT_GIT_REMOTE_URL="https://github.com/soloagency/outreach.git"
...
be nested inside. Run once with the OutreachCRM GitHub remote URL, for example:
  ...
  OUTREACHCRM_GIT_REMOTE_URL=https://github.com/soloagency/outreach.git bash deploy-outreachcrm.sh --git-only
...
generate_skill_manifests() {
  # Each skill folder carries its own LOAD_MANIFEST.md canary. Regenerate it here so a
  # skill edit can never leave a stale canary that would create an unresolvable ledger loop.
  local pb_dir="$1"
  local skills_dir="$pb_dir/skills"
  [ -d "$skills_dir" ] || return 0
  local skill_dir sm rel lines sha last
  for skill_dir in "$skills_dir"/*/; do
    [ -f "${skill_dir}SKILL.md" ] || continue
    sm="${skill_dir}LOAD_MANIFEST.md"
    {
      echo "# LOAD_MANIFEST — full-load reference (canary) for this skill"
      echo
      echo "Auto-generated by deploy-outreachcrm.sh on every deploy. Do not edit by hand."
      echo "After loading any module below, its actual LINE COUNT must match its row here (see playbooks/LOAD_LEDGER_PROTOCOL.md). A shortfall = truncated = NOT loaded; re-read to EOF before acting. last_line + sha256 are OPTIONAL deeper checks, not required every load."
      echo
      echo '| module | lines | sha256 | last_line |'
      echo '|---|---|---|---|'
    } > "$sm"
    while IFS= read -r f; do
      rel="${f#$skill_dir}"
      case "$rel" in LOAD_MANIFEST.md) continue ;; esac
      lines="$(awk 'END{print NR}' "$f")"
      sha="$(sha256_for_file "$f")"
      last="$(awk 'NF{l=$0} END{print l}' "$f")"
      last="${last//|/\\|}"
      printf '| %s | %s | %s | %s |\n' "$rel" "$lines" "$sha" "$last" >> "$sm"
    done < <( find "$skill_dir" -type f -name '*.md' | sort )
    log "Skill manifest written: $sm"
  done
}
