...
plain meter line above; it never repeats the full block for the same transition. The first approved item a counted plan
holds for tomorrow (`contact lock-status` → `writes_today.offer_upgrade`) is the other SELLING trigger and
spends the same once-per-session budget on its own reason. `seat_limit`, a stale-token
`solo_feature_not_in_tier` and `write_via_approval_only` are never a selling moment (Upsell rule carve-outs).
