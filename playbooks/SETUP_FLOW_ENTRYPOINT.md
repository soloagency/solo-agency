...
6. Create or update client setup, default sources, custom sources, found-source approval state, extension folders, collector config, schedule files, automation manifests, scheduled prompts, update-watch state, and resync logs. Discovered sources are never part of Setup Flow — they appear later, as runs find them (`playbooks/10_LEAD_COMPETITOR_DETECTION.md`, "Step 5"; `playbooks/00_CORE_CONTEXT_REQUIREMENTS.md`), for the human to approve.
7. Do not run source scans of any kind, reports, first agency runs, production, rendering, publishing, analytics scans, or outreach in Setup Flow. One narrow exception: the step-7 found-sources DISCOVERY pass may run inside Setup Flow when the human approved it in this session and the Local Collector plus the matching client extension are verified healthy — Local Collector only, approved categories only, output limited to the approval shortlist and saved source configuration. Even then, Setup Flow must not analyze the collected data or produce any report/idea/draft from it. A "no custom URL" answer at step 5 never declines this step-7 discovery pass — the two questions are independent (close gate: `playbooks/10_LEAD_COMPETITOR_DETECTION.md`, "Close gate").
...
19. Roadmap step 5 (Sources) builds the keyword bank per channel FROM `buyer_profile.types` — the channel table, generation procedure, and quality gate in `playbooks/00_CORE_CONTEXT_REQUIREMENTS.md`, "Buyer Profile Channel Keyword Table" — not from intent phrases alone. A "no custom URL" answer here answers only the custom-source question; it never declines step 7's keyword group discovery.

## Kết nối Facebook, Instagram and X (step 4)

This step runs immediately after profile inference (roadmap items 1-3) and before Sources/keyword bank (roadmap item 5). It replaces the old positioning where the bridge/extension install and the Login Reminder sat inside "Required Setup Output" right before dispatch — they now happen here, early, so `facebook_lead_source` (with `instagram_lead_source` and `x_lead_source` alongside it) is known well before the automation task and the Review found sources checkpoint (step 7).
...
4, right after profile inference and before Sources, the automation task, and the
Review found sources checkpoint (step 7). It fires again later only inside a scheduled/automation run whose
...
3. Say that the first run has been dispatched, naming the task, and that the first run reads all of this client's sources — default and custom; if Facebook, Instagram or X are not connected, add that they are off until you connect them, so lead counts will be lower. If it could not be started, give the exact task name for the human to run and why.
4. Say, in the same breath: "I will report the result right here as soon as it lands — it started at {HH:MM} and should finish {HH:MM}–{HH:MM} ({calls_planned} calls × 40–60 s + 10–15 min)." — that window comes from `tool run-progress eta --calls <calls_planned>` output, quoted, never computed by hand — then give the Slow-on-purpose warning (`playbooks/04_DAILY_SCHEDULE.md`, "Run-now per runtime"): the social platforms are scanned one request at a time with a random 5–10 second pause before each request, and the first checkpoint/rate-limit/logged-out signal on a platform stops that platform for the day, so the run may finish later than the ETA or with one platform missing, which is the design, not a fault — never an instruction to open a new chat or run the task themselves. Then give the three dispatch sentences ("It takes time — I will message you — connect Telegram"): this run needs time because the scan is deliberately slow to keep the Boss's accounts safe; Sam will report progress here after each stage and message the Boss the moment the run finishes; and, reading `daily-content-pipeline/schedule.md` `notification_channel` and the latest notification delivery status in this same turn (Read-Before-Claim Rule) — when `notification_channel: local_path_only`, or WideCast notification is configured without Telegram connected (last delivery email-only), recommend connecting Telegram in WideCast (PDNA, the Notification part) so the finish message also reaches the phone (trigger phrase "turn on notifications" / "bật thông báo"); when Telegram is already connected, say instead that the finish message also reaches the Boss's Telegram. If the native automation UI requires human action to actually start, provide that one exact action in a `**[ACTION REQUIRED]**` block instead.
5. Arm the background wait with `--watch progress` (`SOLO_AGENCY_PLAYBOOK.md`, "Wait and report") so Sam reads `tool run-progress --pipeline {setup-root}/daily-content-pipeline show --client <slug>` and reports progress after each of the six Run Progress Rule stages in this same chat and speaks the First-Run Report on the standup wake, then end with a feature-discovery block introducing 2-3 unused headline capabilities from `playbooks/FEATURE_CATALOG.md` (Feature Discovery Rule) - setup never ends flat. Only when dispatch genuinely failed does this step's close carry a `**[ACTION REQUIRED]**` naming the exact client-specific automation task for the human to run instead.

### Setup-Complete Closing Template (MANDATORY shape)

The FINAL message of a completed setup must open with the first-run status line, then this three-offer block (translate to the human's language; adapt names, never the shape). Listing trigger phrases "for later" is NOT compliance: the message MUST end with exactly one question, and NONE of the three offers may be "run the first report" — Sam already dispatched it in the status line above them.

```text
Lượt đầu bắt đầu lúc {HH:MM}, dự kiến xong {HH:MM}–{HH:MM} ({N} lượt gọi × 40–60 giây + 10–15 phút).
Lượt này cần thời gian vì tôi cố ý quét chậm để an toàn cho tài khoản của anh: Facebook, Instagram và
X được quét tuần tự, mỗi request giãn ngẫu nhiên 5–10 giây; nền tảng nào báo checkpoint hoặc giới hạn
tốc độ thì dừng nền tảng đó hôm nay, nên có thể xong muộn hơn dự kiến. Tôi sẽ báo tiến độ ngay tại
đây sau mỗi giai đoạn và nhắn anh ngay khi xong[, kèm tin báo qua Telegram của anh — when Telegram is
connected]. [Để không phải ngồi chờ, anh kết nối Telegram trong WideCast (PDNA, phần Notification) —
xong là có tin báo trên điện thoại; nói "bật thông báo" là tôi cài ngay. — when not connected] Lead
tìm được sẽ vào thẳng CRM chung, và gói Free đang mở sẵn một số lượng contact đầu tiên miễn phí.
...
The two bracketed variants in the first block are chosen by state, never both spoken and never printed with their brackets: read `daily-content-pipeline/schedule.md` `notification_channel` and the latest notification delivery status first (Read-Before-Claim Rule) — "kèm tin báo qua Telegram" when Telegram is connected, the WideCast (PDNA → Notification) recommendation otherwise, and under that second condition offer 2 is the Notifications feature ("bật thông báo" / "turn on notifications").
...
Hard gates on this template: the first line ALWAYS states the first run's state — dispatched-and-waiting, or, only on real failure, the `**[ACTION REQUIRED]**` naming the task (a scheduled future run is never a substitute); the status line carries a start time and a finish window, never a fixed duration; one offer is ALWAYS from the OTHER product side (content setup offers Outreach, Outreach setup offers content/video — the cross-introduce rule); no offer tells the Boss to open another chat or run the task themselves — that work is already dispatched; the closing line is a QUESTION. The template may not be spoken while `facebook_group_discovery.candidates_total > 0` and `review_state == none` — present the shortlist first (`playbooks/10_LEAD_COMPETITOR_DETECTION.md`, "Close gate"). A setup-complete message whose last line is not a question is a Stage-9 audit failure. The priming clause folded into the status line (leads land in the CRM; Free keeps a first batch of contacts open) is spoken in the human's own words and language like the rest of the block, never this fixed sentence verbatim, and never with an estimated contact count — the shape stays one status line, three offers, and one closing question; it does not become a fourth offer or a second question. The Standing Invitation line shown above the closing question (`SOLO_AGENCY_PLAYBOOK.md`, "Standing Invitation") is its own short line, freshly worded each time, placed immediately BEFORE that closing question — never merged into it, never after it, and never counted as the one required closing question itself. One more hard gate: the first block names (a) that the run needs time for account safety, (b) that Sam will message when it finishes, and (c) either the Telegram recommendation or the "also reaches your Telegram" line — a closing message missing any of the three is a Stage-9 audit failure.

Do not ask whether to run the report now. Do not load `playbooks/SCHEDULED_RUN_ENTRYPOINT.md` inside the setup chat. Do not perform source research or source collection of any kind (one exception: the step-7 discovery pass per item 7 of the Setup Flow Contract), report generation, idea matrix updates, Lead & Competitor Opportunities, draft generation, analytics scans, or notification delivery (one exception: the single step-8 WideCast confirmation ping that verifies the notification channel right after the human provides the API key) in Setup Flow.
