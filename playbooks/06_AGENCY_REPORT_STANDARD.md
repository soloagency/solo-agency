...
Any `127.0.0.1` URL (not only the port above) is also a Local Collector reference and must be scrubbed the same way.
...
- The meter line (`playbooks/10_LEAD_COMPETITOR_DETECTION.md`, "Capture never stops at the plan's contact cap") whenever `locked > 0` or the approaching-cap condition is met — the persistent one-line fact, not an ask.
- A "next jobs" block of 2-3 offers chosen, by the STATE POLL, from `playbooks/NEXT_JOB_CATALOGUE.md`, plus exactly one closing question, whenever no `**[ACTION REQUIRED]**` block is already carrying the ask for this run.

Both are operator-only: they stay inside this `INTERNAL_REPORT` section and never appear in the three client-facing HTML files, the combined client report/PDF, or the client notification.
...
This section is required, not optional: it is the only place a human can see whether the keyword bank
is still learning. Open it with one line of bank movement — how many terms ran, how many were short
(1-3 words), how many were retired this run, how many were pinned — then list the keywords.

- Bank movement (from `tool public-keywords stats` and the run's `record` output): {n} keywords run, {n} short, {n} to probation, {n} retired, {n} pinned, {live} live of {total} in bank, short ratio {p}% — and any `problems` the tool listed
- Keyword:
  - Words: {count}
  - Group: industry_general | pain_point | need_or_goal | buying_intent | local_context | related_industry | trend_news | objection | comparison | question | problem_issue   (the bank's `group` field)
  - Search tool used: Google Search | web search | other
  - Query as issued: {the plan's `query` field — the term plus the month/year the tool appended for a time-bound group}
  - Status: used | useful | weak | retry_later | retired
...
1. `People To Contact Today`
   ...
   - INTERNAL_REPORT only: add "{L} locked by plan — read from `contact lock-status`" to this same
     list. Never in the client-facing report or PDF (client-blind rule).
