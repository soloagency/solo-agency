...
- **A failed process start never proves a remote runtime.** `CreateProcess`, `ENOENT`, `No such file or directory` or `cannot run program` usually means the working directory does not exist or the shell path is wrong. Check the directory, create it from its nearest existing parent, check the shell path, retry once — and only call the runtime remote when the local filesystem itself is unreachable after that (full rule: `playbooks/SETUP_FLOW_ENTRYPOINT.md`, "Process-start failure is not a remote runtime"). Handing the human a command after one failed spawn is the bug this rule exists to stop.
...
- That one unpacked extension folder covers all three social platforms — it is not per-platform. Internally it carries separate `platforms/facebook`, `platforms/instagram`, and `platforms/x` modules, but the human installs it once (setup step 4, "Kết nối Facebook, Instagram and X") and it reads whichever of the three platforms the human is logged into in that Chrome profile. The Login Reminder likewise names all three platforms in one prompt, not three separate reminders. Immediately before that existing reminder/action block, setup renders the informational `assets/theloop_light.png` image inline using the verified absolute setup-root path (`playbooks/SETUP_FLOW_ENTRYPOINT.md`, "Setup-chat visual contract"); it never replaces or delays the extension tab/action.
...
After bridge `GET /status` succeeds and proves current-workspace identity, automatically print and show `http://127.0.0.1:17321/ui/{client_slug}#revenue-engine` under the SHOW RULE, then open `/ui/{client_slug}/extension` as the foreground tab for the extension action. Keep Revenue Engine behind while the extension installs/checks in (or show it first then remain on Extension with only one tab); bring it forward only after `extension_health.status: recent`. This read-only localhost dashboard display is automatic, not an `**[ACTION REQUIRED]**` block, and is never private-source browsing.
...
- During Setup Flow, do not run a report. Prepare or resync the client-specific automation task whose task name begins with the client name, then ask Step 7's one run-now question and dispatch only on yes.
- After that yes, create/start the separate one-time `{Client} - Solo Agency First Run` task autonomously; never reuse the recurring Daily Run or ask Codex/Claude users to click Run now/open a scheduler. Record lifecycle and delete/mark deleted after the First-Run Report.
