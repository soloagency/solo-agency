...
- When asking for a WideCast API key or describing what it unlocks, say that WideCast enables video/blog/social asset creation and notifications for human review, then publishing to 10+ platforms only after the human approves the exact content and target platforms. Do not imply that the API key alone authorizes unreviewed automatic posting. Add, in one clause, that the same key is the Solo Agency license: the Free plan needs no key at all, and the WideCast plan on that account (Starter / Pro / Business / Enterprise) sets the Solo Agency tier — more unlocked CRM contacts (every data feature is on every plan; group posting, commenting and reacting start at Starter) (`AGENTS.md`, "Plans").
...
**Also:** this key is your Solo Agency license — Free needs no key at all, and the WideCast plan on this account (Starter/Pro/Business/Enterprise) is what sets how many CRM contacts stay unlocked (every data feature stays free on every plan).
