...
- Before claiming any post-schedule change is complete, verify Automation Resync was performed when schedule/automation exists. Config-only updates are not enough if a native scheduled task prompt may still contain an old snapshot.
...
- Every affected `extensions/{client_slug}_extension/` folder was regenerated or patched when extension code/templates changed.
...
- Updating the extension template but not updating per-client `extensions/{client_slug}_extension/` folders.
...
4. **Kết nối Facebook, Instagram and X.** Install the Local Collector bridge (agent-run on a local runtime after one consent line, handed off on a remote runtime) and this client's extension — one extension, all three platforms — via the two-gesture install (90-second help loop if check-in stalls: Developer mode, wrong Chrome profile, or Chrome closed — 3 rounds). After a successful current-workspace `/status`, automatically show Revenue Engine once, then leave the Extension page foreground until `extension_health.status: recent`, when Revenue Engine returns to foreground; this localhost dashboard display is read-only and not an action request. Then deliver the Login Reminder (naming Facebook, Instagram, and X) and record `facebook_lead_source: enabled|web_only|pending`, with `instagram_lead_source` and `x_lead_source` following automatically (`web_only` only on an explicit acknowledgment, never "để sau"). A still-`pending` item 4 does not block onboarding — continue to step 5 — but the client's first dispatched run is gated on `facebook_lead_source` being `enabled` or `web_only`, never `pending` — Instagram and X follow automatically.
5. **Sources.** Automatically set up the default sources (Google search with the keyword bank, Facebook, Instagram and X through the connected extension, and this client's industry sites) and save the source plan and keyword bank. Do not ask for custom sources or create an `**[ACTION REQUIRED]**` block; preserve any URL voluntarily supplied during setup and process it when the collector is ready.
6. Configure the recurring schedule/routine once the basic source plan is known, prepare or verify the client-specific extension folder under `extensions/{client_slug}_extension/`, and create/resync the client-specific automation task with the task name beginning with the client name. With no explicit saved or Boss-provided schedule, automatically use daily 09:00 local through `schedule-slots suggest` → returned actual time → `register`; report the actual time/finish window and say it can be changed later. An explicit schedule wins; repair/update preserves one; if the tool is unavailable, use 09:00 local (or explicit override), set `slot_check_pending`, and register later. Configure the initial task with default sources plus any active custom sources voluntarily supplied during setup; no custom-source response gates it.
7. **Chạy lượt đầu (First run).** Once the recurring Daily Run exists, ask the Boss exactly one question, translated to their language: may I run the first report now. The Local Collector was already installed at step 4. On yes, record `first_run_consent: yes`, autonomously create/start the separate one-time `{Client} - Solo Agency First Run`, and resync the task contract; Daily Run stays distinct, lifecycle is recorded/cleaned up, and Codex/Claude never ask the Boss to operate the scheduler. Joined-places discovery rides the same yes as `approved_pending_first_scan` for all categories; the Group Potential Rule sends high/medium groups directly to monitoring and low groups to `not_selected`, with no shortlist or separate approval. On "để sau" / not now, record `first_run_consent: not_now`, resync after the decision, and leave the Daily Run schedule unchanged.
...
If the human asks to run, create, generate, show, refresh, or update a report during Setup Flow, treat it as a failed-safety condition unless the agent stops operational work. Verify/resync the contract, then only after Step 7 yes autonomously create/start the separate First Run in its own session; only a genuinely incapable OTHER runtime may name it for human start, while Codex/Claude recover/escalate without scheduler-click handoff. Avoid loading the scheduled-run entrypoint or running report work inside the setup chat.
...
The agent must configure the recurring `Nguyen Law - Solo Agency Daily Run`, then ask Step 7's one first-run question and on yes autonomously create/start the separate `Nguyen Law - Solo Agency First Run`, wait/report/clean it up. Codex/Claude never hand the start to the human; Setup Flow must not run Nguyen Law's report directly.
...
The one allowed private data source choice is operational, not a new menu: after the automation task exists, if private data sources exist but the Local Collector (installed back at step 4, Kết nối Facebook, Instagram and X) is unhealthy, mark private data sources pending so the automation task can run public data sources only until the blocker is resolved.
...
5. Ask step 7's one first-run question (Chạy lượt đầu) once the automation task exists. The Local Collector was already installed at step 4 above; on yes, dispatch the first run immediately — joined-places discovery and group monitoring happen automatically inside it, never as a separate approval.
...
- The first report happens in the client-specific automation task, after routine setup, task creation, and step 7's one first-run question (Chạy lượt đầu) — dispatched immediately on yes, or left on its existing schedule on "để sau" / not now.
- Setup Flow must not create `/jobs/run_now`, must not scan default sources, and must not scan custom sources merely to produce a report. Step 7's one first-run question (Chạy lượt đầu) dispatches the actual first run as its own automation-task session on yes — that dispatched session is where scanning, joined-places discovery, and the report happen, never inside the setup chat itself.
- If custom sources were provided and the Local Collector (installed at step 4) is not healthy, the setup agent must still ask step 7's one first-run question (Chạy lượt đầu) after automation exists, recording custom sources as pending, then resync the task.
...
If any voluntarily supplied custom source is waiting for the Local Collector, this run uses the default sources only. {N} custom sources are pending, including:
- {source name or URL}
- {source name or URL}
...
- Collector setup is no longer gated on custom sources — it already happened at setup step 4 (Kết nối Facebook, Instagram and X), before the automation task and automatic step 5. This section covers what step 7 (or a later run) does when it needs to check or repair that install, not a fresh first install.
- **Local runtime** (the agent can see the install root on its own filesystem — Codex CLI, Claude Code desktop/CLI, Hermes, OpenClaw, or a comparable local runtime): the agent writes the script/launcher file, runs the Stage 8 Source Safety Pre-Check, gives the ONE short plain-language safety confirmation line, asks for consent once, then RUNS `setup_collector.sh`/`setup_local_collector.ps1` itself and waits for `GET http://127.0.0.1:17321/status` to answer (up to 60 seconds). The setup script hands the process to an OS-level autostart service (macOS LaunchAgent, Linux systemd user unit, or a Windows logon Scheduled Task) — the agent's own shell/session never owns that process, so a killed agent turn does not stop the bridge; this is why the old "the agent must never execute the setup script" rule no longer applies here.
- **Remote runtime** (a hosted sandbox where `127.0.0.1` is not the human's own machine): detected when the agent cannot see the install root on its own filesystem, or `/status` still fails 60 seconds after a bootstrap attempt. Only then does the agent fall back to preparing the script/launcher file and handing the human exactly one short Terminal/PowerShell command or one double-clickable file path to run outside the sandbox, not a long multi-line script.
- The Stage 8 Source Safety Pre-Check runs before any install action (agent-run or handed off): the prepared extension JS and `prepare_client_extension.sh` were read (the bridge is a checksum-verified binary whose source is not in the repo — its checksum was verified, never claimed as read), every outbound request from the extension was confirmed to go only to the local `127.0.0.1` bridge, and the result was recorded in `INTERNAL_REPORT`. If the pre-check does not pass, no install action is taken; the finding is raised to the operator instead.
...
- The handoff must say which Chrome profile/account to open for that client (first client: the profile already in use; a second profile only when a second client needs a different Facebook account).
...
- The agent must create or update `daily-content-pipeline/collector/collector_setup_status.md` at step 4, when the install runs, not only when private data source activation begins.
...
- After collector setup succeeds, no discovery runs in the setup chat: the first run (step 7, on the Boss's one yes) executes joined-places discovery and the Social Discovery Pass, judges every readable group with the Group Potential Rule and monitors high/medium ones automatically; the run reply lists them with their potential and the Sources page link (the Boss pauses there).
...
- On a fresh setup with no explicit saved or Boss-provided schedule, automatically create daily 09:00 local using `schedule-slots suggest` → returned actual time → `register`; report the actual start time/finish window and say it can be changed later. Do not ask a schedule question or use an `**[ACTION REQUIRED]**` block. An explicit schedule wins; repair/update preserves one.
...
- During Setup Flow, do not run a report. Finish by preparing or resyncing the client-specific automation task whose task name begins with the client name, then ask Step 7's one run-now question and dispatch only on yes.
- After schedule/routine setup and automation task creation, ask step 7's one first-run question (Chạy lượt đầu) once. The Local Collector was already installed at step 4; if it is not healthy, clearly mark custom sources as `pending_private_activation` in the automation contract so the first automation run can continue with default sources only if needed, then resync the task.
...
3. For each active client:
   ...
   11. Check private data sources if available, using the Solo Agency Local Collector extension plus the Local Collector app when available, with `collector_config.scroll_delay_seconds` defaulting to 5 seconds and `collector_config.max_scrolls_per_source` defaulting to 5. Shared-Scan Gate first (playbook 08): `tool source-registry due` decides which sources actually scan and which reuse another subscriber's fresh scan via its data pointer; record outcomes back after the run.
   ...
   14. Load the private data stored by earlier completed runs for this client when available — at minimum the previous completed run (per Stage 4's Run Window anchor: located from run history on disk, never `today − 1 day`) — and filter duplicates: records carrying `point_uid` dedup by key equality first (exact, cross-client-safe), then visible text matching for near-duplicates. Do not parse private-platform HTML for duplicate detection.
...
9. If the configured provider notification capability is available, preferably WideCast OpenAPI `sendNotification`, upload the scrubbed client-facing HTML report first when an HTML-capable upload operation such as `uploadAsset` is available, upload the PDF companion too when the verified client provider supports PDF upload, then send the notification composed per the Client Notification Contract (playbook 03): the notification channel belongs to the client's account, so the body carries the uploaded report URL, valuable findings, plain numbers, and client-safe coverage gaps — while run status, clients processed, operator blockers, INTERNAL_REPORT path/status, and required operator actions go to the run output, notification log, and INTERNAL_REPORT.
...
- Processed:
- Skipped:
- Sources needing login not yet connected:
...
```md
Add this client to the daily content pipeline: Nguyen Law, immigration lawyer in San Jose. Custom sources to monitor: [links]. Infer everything else and show me the setup summary before saving.
```
...
1. Smith Law - DUI lawyer - Los Angeles - custom sources: [links]
2. Austin Home Group - real estate agent - Austin, TX
3. Bright Mortgage - home loans - Texas - custom sources: [links]
```
...
```md
What product/service, profession, expertise, business description, or public website/profile URL should this pipeline focus on? If location matters, include the target location. I can use a public website/profile URL to understand the business if you prefer. I will infer industry, audience, pain points/customer problems, content pillars/main content themes, and default sources (web search I can access without your login), then show you the setup summary before saving anything as stable context.
```
...
If the human says they have no clients yet, or if the first run discovers that `clients_index.md` has no real client rows, the agent should create or verify the root structure and immediately enter First Client Setup Mode. It should ask only for the first client's name and product/service, profession, expertise, business description, or public website/profile URL, plus target location only if location matters and cannot be inferred. It must not mention custom sources in the first client setup question. Step 5 automatically configures default sources and keyword banks; any URL volunteered during setup is preserved and processed when the collector is ready. Later, custom sources are added with Revenue Engine `watch_source` or a direct request to Sam, followed by Automation Resync.

If the human gives a new client, the agent should enter Add Client Mode.

After Add Client Mode or First Client Setup Mode, the agent must follow the fixed order: setup context, install the Local Collector bridge and extension (Kết nối Facebook, Instagram and X), configure the recurring Daily Run, ask Step 7's one first-run question, then on yes autonomously create/start the separate one-time First Run, wait/report/clean it up. Setup Flow must not jump into report generation, video creation, publishing, or production actions.
...
12. The matching client extension folder exists under `extensions/{client_slug}_extension/`, with the client name at the start of the Chrome extension name, and the setup handoff shows the absolute folder path plus the exact Chrome `Load unpacked` steps for the matching client Chrome profile/account.
13. Step 4 (Kết nối Facebook, Instagram and X) ran before the automation task and the private data source checkpoint: the Local Collector bridge answered `GET http://127.0.0.1:17321/status` — installed and started by the agent itself after one plain-language consent line when running on a local runtime (waiting up to 60 seconds), or handed to the human as a one-line command on a remote runtime that could not see the install root or verify `/status` in that window. `daily-content-pipeline/collector/collector_setup_status.md` reflects the real outcome (`installed_and_running` or a precise blocked status with the required human action) — this is unconditional, not gated on the human wanting private data sources monitored.
14. The client's Chrome extension check-in was verified (`extension_health.status: recent` within the 75-second grace window) at step 4, or the human explicitly confirmed `web_only` there (an acknowledged choice, never a bare postponement), or item 4 is still `pending`; `facebook_lead_source: enabled|web_only|pending` is recorded in the Client Intelligence Profile either way, with `instagram_lead_source` and `x_lead_source` following automatically, and the first dispatched run (item 19 below) never fires while `facebook_lead_source` is still `pending`.
15. If custom sources voluntarily supplied during setup exist but are not active yet, the automation contract includes `Private Data Sources Pending Activation` and lists the pending sources.
16. If no custom sources were voluntarily supplied, setup remains complete; source discovery rides the first-run decision without a separate source question.
17. If no private data sources are active, the automation contract includes `Private Data Source Discovery Recommended` or `Private Data Source Discovery Declined/Postponed`, with a plain note that public-only reports can miss community, lead, and competitor signals.
...
2. With no explicit saved/Boss schedule, the recurring Daily Run was automatically configured daily at 09:00 local; an explicit/saved schedule wins and is preserved before the client-specific automation task is marked ready.
...
24. Human approval options are shown where a decision is pending (content, publishing, comment-thread harvest); monitored groups never need one — they are listed with their potential and the Sources page link.
...
- [ ] If this reply answered a Boss information question, did I also navigate the side dashboard (or, on a remote runtime, print the link) to the routes-table view that matches it, per the Answer-and-Show Rule (`SOLO_AGENCY_PLAYBOOK.md`, "Team Leader Reply Frame", `playbooks/TEAM_MODEL.md` duty 6)? An information answer with no side-pane navigation and no printed link on a remote runtime is a finding, and so is a navigation to a path not listed in `tool ui routes` / `GET /api/ui/routes`.
...
- [ ] Does this reply use exactly one closing branch: either `**[ACTION REQUIRED]**` block(s), with no Revenue Engine anchor/feature tour/next jobs/optional CTA anywhere in that reply; or the no-required-action sequence ending with next jobs plus exactly one closing question? Never both, never neither, never a passive sign-off.
- [ ] When `contact lock-status` shows `locked > 0`, does this reply (and INTERNAL_REPORT / morning brief, where applicable) carry the persistent one-line meter ("{L} leads locked under {tier} - {unlocked}/{max} open")? When `locked == 0` and `contact lock-status` reports `approaching: true`, does it carry the approaching-cap line instead, and does the reply name the upgrade tier only from `next_tier`, never from memory?
...
- [ ] If this session already showed the `review_locked_leads_upgrade` `**[ACTION REQUIRED]**` block once, did I avoid showing it again this session (carrying only the meter instead), and is `write_allowance_upgrade` shown only after an approved item was really held for tomorrow (`writes_today.offer_upgrade`), never speculatively?
- [ ] On a plan that counts writes (`contact lock-status` → `writes_today.capped`), did every Boss-facing reply carry the write meter line verbatim, and did I say that held items publish tomorrow on their own?
...
- [ ] For every order brief under this client's `orders/` (`playbooks/ORDER_RULE.md`): a run-log row for this run (ran, or skipped with a reason); the report's Orders section with one line per active brief; every write the order produced at an approval point, never published from the run; every capability the brief names present in the catalog; the notification line carrying the order's output.
- [ ] On every post-setup interactive reply with no required action and no complete 25-job menu already shown, did I include the compact Revenue Engine awareness anchor after any feature-tour block and before next jobs, with the current client link (or `show me the features` fallback), without auto-navigation? Did I omit it from First Words, Setup Flow before completion, scheduled/provider notifications, INTERNAL_REPORT, and client-facing files?
- [ ] At setup-complete, a no-pending-action handoff, a lead-detected run, or the weekly cadence, when no `**[ACTION REQUIRED]**` block was present, did I include a feature-discovery block (Feature Discovery Rule) drawn from `playbooks/FEATURE_CATALOG.md` - unused features only, value-first with the exact trigger phrase, at most 2-3, not repeating one surfaced last message, and Outreach on top when leads were found? When action was required, did I defer rather than stack it?
- [ ] If this message completed a setup: does it follow the Setup-Complete Closing Template - first line states the first-run status (dispatched / in progress / reported), no offer tells the Boss to open a new chat or type run task, one offer from the other product side, and is the LAST line exactly one question?
- [ ] Right after dispatching a run, did I arm the one background wait where the runtime supports it (`playbooks/04_DAILY_SCHEDULE.md`, "Wait and report"), or, on a runtime without background execution, state the expected duration and the exact phrase to send instead - never a foreground sleep, never a poll loop in the chat?
- [ ] When the background wait woke me, did I deliver the First-Run Report in the SAME chat, within the ≤ 8 line shape (finished-at, lead counts plus the locked-contacts meter line, needs_boss items, the report opened beside the chat with the link printed, next-job offers, one question)?
- [ ] On a local Claude Code runtime, did setup step 6 ask the unattended-permissions consent exactly once, right after the client-specific automation task was created, and record `unattended_permissions`, `unattended_permissions_scope`, and `unattended_permissions_written_at` on the manifest either way (granted with the rule set written, or declined with the one-sentence pause explanation, or not_applicable on a non-local/remote runtime — `declined` and `not_applicable` are never the same outcome)?
- [ ] Did First Run use a separate one-time task (never Daily Run), record name/id/runtime plus `created|started|running|completed|reported|deleted|failed` lifecycle and idempotent stale/running handling, then delete/mark deleted after report while Daily Run persists?
- [ ] Does every shell command in the scheduled-run prompt/task follow the Command Shapes rule (`playbooks/SCHEDULED_RUN_ENTRYPOINT.md`) - only `<bridge binary> tool ...`, `solo_tool ...`, `wait_for_run ...`, or a flag-then-URL `curl` call, never `python3`/`node`/`bash -c`/`sh -c`/pipes/`&&`/`xargs`/`find -exec`/`sed -i`, and file writes only under `daily-content-pipeline/` and `extensions/`?
- [ ] If this is a Setup Flow message that asked the human for anything - a question, a choice, a consent, an `**[ACTION REQUIRED]**` gesture, a paste - does it close with a Standing Invitation line (`SOLO_AGENCY_PLAYBOOK.md`, "Standing Invitation"; `playbooks/SETUP_FLOW_ENTRYPOINT.md` item 16), freshly worded and never the same fixed sentence twice in a row, sitting BEFORE the closing question rather than after or merged into it? A Setup Flow message that asks for something without this line is a finding.
- [ ] Did I keep the most important required action at the end of the reply, with no more than three `**[ACTION REQUIRED]**` blocks and no Revenue Engine anchor, optional feature prompt, next-jobs offer, link, or question after it?
...
- [ ] If the latest GitHub version still did not resolve the blocker, did I report it on the first rung this install can reach (WideCast `reportError`, else the Solo Agency support group, else a GitHub issue only where an identity already exists, else a draft) without ever asking the human for a GitHub account, record the channel used (WideCast `request_id`, support-group post, issue URL/number, or draft path) in `daily-content-pipeline/automation/github_issues.md`, and tell the human how it will be tracked?
...
- [ ] Did I actually RENDER the human-facing report to HTML (plus PDF) this run and hand THAT off — never producing only a `.md`, never ending by pointing the human at a `.md`, and on a render failure surfacing the exact blocker instead of handing over the `.md`?
...
- [ ] Did every readable group found this run get a `potential` (high/medium/low) with a one-line reason — high/medium registered as `active`, low as `not_selected` — with no approval question to the Boss?
- [ ] Did the run reply list the newly monitored groups with their potential and the Sources page link, instead of an approval block?
- [ ] Did I scan at most 20 monitored groups this run, in the order of `tool source-registry plan`, and record leads per group with `record --leads`?
- [ ] While this client's registry plan was in the sweep phase (`"phase": "sweep"`, any active group with `scans == 0`), did I dig the never-scanned groups first (newest registered first) rather than the leads-ranked order, per "Group sweep before optimisation" (`playbooks/10_LEAD_COMPETITOR_DETECTION.md`)?
- [ ] Did the Group Potential Rule prefer groups/communities and reputable organization pages and default-skip individual personal profiles (`skip_individual_profile`), keeping an individual account only with recorded high-reach-authority evidence — not filling the list with low-signal personal profiles that are really leads?
...
- [ ] If this was one-time Local Collector setup/update/repair on a LOCAL runtime (I can see the install root on my own filesystem), did I run `setup_collector.sh`/`setup_local_collector.ps1` myself after the one plain-language consent line, and wait for `/status` (up to 60 seconds), rather than needlessly handing the human a command I could run myself?
- [ ] If this was one-time Local Collector setup/update/repair on a REMOTE runtime (I cannot see the install root, or `/status` still failed 60 seconds after a bootstrap attempt), did I give the human both required local actions in chat: run the one-line Terminal/PowerShell setup/start command outside the sandbox and load the Chrome extension from the absolute runtime folder — rather than falsely claiming I can run it myself?
...
- [ ] Did I infer `buyer_profile` (`sells`, `sells_to`, `types`, `why_they_need`, `location`, `competitors`, `not_buyers`) at the same step-2 moment, is every `types` line CATEGORY FIRST with examples only after `e.g.` (never a closed list), and did I get the Boss's one-sentence confirmation before treating it as stable?
...
- [ ] Did I keep human-facing text to exactly two source words — default sources and custom sources — treating any appearance of `public data sources`, `private data sources`, `public/private`, `lane`, or `logged-in sources` in what the human reads or Sam says as a finding?
- [ ] Did I treat step 6 as schedule/routine plus client-specific automation task setup, not as a source-preference gate?
- [ ] Did I keep step 5 automatic — default sources and keyword banks configured with no custom-source question, no required action, and no gate on steps 6-7 — while preserving any volunteered URL?
- [ ] Did step 7 (Chạy lượt đầu) fold joined-places discovery into the same one first-run question — groups, subreddits, communities, followed profiles/pages/KOLs, subscribed channels, and feeds — with no separate discovery question, then resync the automation task?
- [ ] Did step 7 (Chạy lượt đầu) dispatch the first run immediately on yes, with joined-places discovery riding the same yes and filtered by the Group Potential Rule with no separate human approval (or record `first_run_consent: not_now` when the human said "để sau")?
- [ ] Did I ask the step 8 PDNA notification question during setup with the value-first framing (hot-lead alerts, report-ready, drafts awaiting review), without pressure language and without implying any Solo Agency-provider affiliation - or record the decline as `notification_channel_missing` for the once-per-run re-offer?
- [ ] If this was Add Client Mode or First Client Setup Mode, did I create or verify a dedicated `extensions/{client_slug}_extension/` folder, patch the Chrome extension name to `{Client Name} - Solo Agency Collector`, and show the absolute folder path plus exact `chrome://extensions` -> Developer mode -> `Load unpacked` steps for the matching client Chrome profile/account?
- [ ] Did I avoid asking about the browser or the profile entirely, and hand over the video plus the extension folder path instead?
- [ ] Did I show the install video from the dashboard page or the bridge's embedded `/ui/assets/help/...` URL, checking the on-disk copy exists before naming it, and never a GitHub link on a local runtime?
- [ ] Did the agent's `POST /api/ui/{client_slug}/install-extension` call carry no `browser`/`profile_directory` fields, and did I tell the human to open their own browser's extensions page regardless of which one the best-effort open landed on?
- [ ] If only Safari or Firefox was installed, did I tell the human to install Chrome (free; using it just for Facebook is fine) instead of trying to load the extension into an unsupported browser?
...
- [ ] Did I avoid joining groups, requesting access, messaging admins, or monitoring a group the Group Potential Rule rated `low` or that the account cannot read (`no_access`)?
...
- [ ] Did I judge every joined Facebook group with the Group Potential Rule (high/medium monitored, low recorded as `not_selected` with a reason) instead of monitoring all of them blindly or asking the Boss to approve them?
...
- [ ] Did I check every bank (public web, in-group, community discovery) against the quality gate in `playbooks/00_CORE_CONTEXT_REQUIREMENTS.md`, "Buyer Profile Channel Keyword Table" — no `need_or_goal`/`buying_intent` term standing alone with no role or offer anchor ("advice", "looking for", "need help", "any recommendations", "content help" alone are all rejected)?
...
- [ ] If this is one-time setup/update/repair, did I follow the local/remote rule correctly: start/restart the Local Collector app myself when I am on a local runtime that can see the install root (one consent line, then wait for `/status`), and only fall back to a human-run setup/start command when I am on a remote runtime that cannot see the install root or verify `/status` in time?
...
- [ ] After any collector setup/upgrade this session — and FIRST when diagnosing a bridge that is down after a reboot — did I verify autostart at the deepest rung my sandbox allows (OS-level check when I can run commands; otherwise read `solo-agency-local-collector/autostart.json` and require `mode != "none"`; otherwise hand the human the per-OS one-liner and ask for the output) and report the result instead of assuming? If `mode` was `"none"` or the file was missing, did I tell the human the fix is re-running the setup script (human-run, one command) rather than hand-crafting a service myself?
...
- [ ] After Facebook joined-groups discovery, did I register the high/medium groups as monitored automatically and list them in the run reply with the Sources page link — no approval block?
...
- [ ] Did I load Stage 10: `playbooks/10_LEAD_COMPETITOR_DETECTION.md`, AND `playbooks/LEAD_QUALIFICATION_RULE.md`, before qualifying any row?
- [ ] Did I treat lead/competitor detection as a core opportunity module, not a small appendix?
- [ ] Did I run `playbooks/LEAD_QUALIFICATION_RULE.md` Step 1 (WHO is this person, against this client's own `buyer_profile.types`) first, on every row from both passes and the feed/surface pass, before deciding competitor/noise or intent?
- [ ] Does every lead row carry `person_type`, `sells_to_match`, `fit`, `fit_reason`, `intent`, `intent_reason` alongside the hot/warm/watch decision?
- [ ] Did I keep a right-type person with no stated need as `warm` rather than dropping them for lacking a stated need?
- [ ] If `playbooks/LEAD_QUALIFICATION_RULE.md` was edited this session, was the regression test in `playbooks/tests/lead-rule/` re-run and did it clear baseline before the edit was treated as live?
...
- [ ] Did I monitor high/medium groups automatically and record low ones as `not_selected` with a reason, without asking the Boss?
- [ ] Did this run scan at most 20 monitored groups in plan order and record leads per group?
- [ ] Did I keep every group that is public or one the account is already a member of (`groups_readable`), mark no-access groups without scanning, joining or requesting them, and register high/medium ones as monitored (`active`) and low ones as `not_selected` with a reason — no human promotion step?
...
- [ ] For every comment-campaign draft/dispatch record, did it carry `person_type`, `sells_to_match`,
      `fit`, `intent`, and `decision` from `playbooks/LEAD_QUALIFICATION_RULE.md`, and did no comment go
      out on a `none` or `competitor` decision unless `goal.description` explicitly allowed presence in
      competitor threads?
...
- [ ] Did I treat step 8 as provider/capability setup after the setup handoff or an Automation Flow report, not open-ended trial video creation?
...
- [ ] Did the client notification follow the Client Notification Contract (playbook 03): hosted links, valuable findings, plain numbers, client-safe coverage gaps, client-owned decisions only — with operator status, blockers, paths, and next actions in the run output, notification log, and INTERNAL_REPORT?
