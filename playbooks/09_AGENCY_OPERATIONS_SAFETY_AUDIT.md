...
7. **Chạy lượt đầu (First run).** Once the automation task exists, ask the Boss exactly one question, translated to their language: may I run the first report now. The Local Collector was already installed at step 4. On yes, record `first_run_consent: yes` and dispatch immediately per `playbooks/04_DAILY_SCHEDULE.md`; joined-places discovery rides the same yes (`approved_pending_first_scan`) with its results filtered by the Group Potential Rule straight into monitored sources — no shortlist, no separate human approval. On "để sau" / not now, record `first_run_consent: not_now` and change nothing else; the daily task keeps its existing schedule. Resync the already-created automation task after the decision.
...
The agent must configure the routine, prepare the `Nguyen Law - ...` automation task, then ask step 7's one first-run question (Chạy lượt đầu) and dispatch on yes, and tell the human to run that client-specific task for the first report only if dispatch was not possible. Setup Flow must not run Nguyen Law's report directly.
...
After Add Client Mode or First Client Setup Mode, the agent must follow the fixed order: setup context, install the Local Collector bridge and extension (Kết nối Facebook, Instagram and X), configure schedule/routine, prepare or resync the client-specific automation task, ask step 7's one first-run question (Chạy lượt đầu) once the automation task exists, resync the task after any source-state change, then hand off the exact task name the human should run for the first report. Setup Flow must not jump into report generation, video creation, publishing, or production actions.
...
- [ ] Did dispatching and waiting on a first run record the four first-run manifest fields as they happened — `first_run_task_id` and `first_run_dispatched_at` at dispatch, `first_run_wait` transitioning `armed` → `timed_out`/`reported` (or staying `not_available` with no background wait), and `first_run_reported_at` when the First-Run Report was actually spoken?
