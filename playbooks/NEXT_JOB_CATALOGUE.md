...
| `research_content_signals` | 4 | Boss asks for comment-backed content research, or asks to strengthen the evidence behind ideas before writing | select recent notable threads, extract useful questions, pains, objections, and opinions into the Content Evidence Bank and refresh the Idea Matrix | "Em nghiên cứu comment từ các bài đáng chú ý để làm giàu Idea Matrix nhé?" | "Want me to research comments from notable posts to strengthen the Idea Matrix?" | `playbooks/CONTENT_SIGNAL_RESEARCH.md` | recent notable posts or an existing Idea Matrix; on demand by default | Free+ |
...
- **`connect_notification`** is also the mandatory offer 2 of the Setup-Complete Closing Template
  (`playbooks/SETUP_FLOW_ENTRYPOINT.md`, "Setup-Complete Closing Template") whenever its condition
  holds — under that condition it is not one of the free feature slots, it is the Telegram
  recommendation folded into the template's first block.
...
- **Content-signal research stays research.** `research_content_signals` is on demand unless the
  Boss explicitly asks for a separate automation. Setup never creates that automation. It loads
  `CONTENT_SIGNAL_RESEARCH.md` only for this job and may update the Content Evidence Bank and Idea
