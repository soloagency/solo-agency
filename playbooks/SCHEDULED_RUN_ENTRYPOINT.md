...
Read only this client's provider config under integrations/providers/. If PDNA actions, analytics, report upload, notification, video creation, or publishing are needed, fetch/cache the configured provider OpenAPI spec, verify the provider account belongs to this client, and stop provider actions on account mismatch. If a global MCP/native provider account is visible but cannot be proven to match this client, log `global_mcp_not_client_scoped` and do not use its credits, platforms, notifications, analytics, or publish settings as this client's state.
