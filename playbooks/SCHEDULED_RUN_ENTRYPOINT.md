...
7. Run the Daily Run in this order (playbooks/AUTOMATION_SCHEDULING.md has the full contract):
   ...
   j. Produce the daily ops report + INTERNAL_REPORT (operator-only). On the weekly schedule, also produce the weekly client report through the Client-Blind Scrub Gate (report_renderer.py package --client-facing --fail-on-scrub).
...
11. Load playbooks/09_OPERATIONS_SAFETY_AUDIT.md before claiming the scheduled run is complete.
```
