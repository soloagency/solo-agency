...
7. Run public research first, then private scans if active, then published-URL analytics only when published URLs/metrics exist, analysis, Lead & Competitor Opportunities, idea matrix, best idea selection, production-ready drafts, approved video/blog/social asset creation when provider setup and explicit approval allow it, HTML report generation, learning updates, and notification. Public data source intelligence and private data source intelligence must be separate lanes inside one canonical report.
...
18. Load Stage 6 and obey the Two-Lane Public/Private Report Contract: one canonical report per client/day/run, public lane above private lane, private pass appends into or replaces only the private lane, and no pass may overwrite the other lane.
19. If public data sources finish before private data sources, generate/update the same report path with `Public Data Source Intelligence` complete and `Private Data Source Intelligence` pending/blocked. If private data sources later finish, update the same report path by appending/replacing only the private lane.
20. Two notifications are acceptable: one for `public lane ready` and one for `private lane appended` or `private lane blocked`. Both must point to the same canonical HTML report path or uploaded URL, and each notification must state the lane status.
21. Run the Report Delivery Capability Check for every report notification or report handoff. Use tool discovery/lazy-load before declaring WideCast upload or Telegram notification unavailable.
22. Prepare a report-delivery record for every report notification/handoff: lane status, local `.html` path, WideCast capability checked true/false, tool discovery method, upload tool available true/false/unknown, notification tool available true/false/unknown, upload attempted true/false, uploaded URL if available, upload blocker if any, notification channel, notification attempted true/false, and final notification report link.
23. If WideCast notification/Telegram is connected and WideCast report/file upload is available, upload the current canonical HTML report to WideCast before each report notification, then send that uploaded WideCast report URL through WideCast Telegram/email fallback. Do not send only a local file path when an uploaded URL is available.
24. If the current AI connector/tool surface does not expose WideCast HTML upload or Telegram/notification tools, log `widecast_report_upload_unavailable` and/or `widecast_notification_tool_unavailable`, say this is a current tool-surface blocker, and notify/surface the best available HTML path/link through chat or another authorized fallback.
...
Generate this client's one canonical HTML report with separate public/private lanes, update this client's history/learning, and resync any configuration changes made during the run.
If private data sources complete after public data sources, append/update only the private lane in the same report and preserve the public lane exactly except for report-level timestamps/status.
```
...
1. Generate or update the local canonical `.html` report.
...
5. Send that uploaded URL through WideCast Telegram with the run summary, lane status, blockers, lead/competitor counts, and next action.
...
7. Log the upload attempt and notification in `daily-content-pipeline/notifications/notification_log.md`, including lane status.

If both public and private lane notifications are sent, both notifications must link to the same canonical report path or uploaded URL. Do not generate a second private report for the private notification.
