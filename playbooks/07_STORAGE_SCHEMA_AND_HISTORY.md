...
- Monitored Facebook groups (`source_type: group`, `kind: private` — read through the human's session) carry additional per-client fields, written only by `tool source-registry add|record|pause|resume` — never hand-edited: `state` (`active | paused | not_selected | no_access` — `active` is scanned by the daily plan, `paused` is held out by the Boss on the Sources page, `not_selected` is a Group Potential Rule `low` verdict, `no_access` is a private group the account cannot read), `potential` (`high | medium | low`, the Group Potential Rule's verdict), `potential_reason` (its one-line reason), `added_at` (ISO-8601 timestamp this group was first registered — ranks the sweep phase, newest first), `scans` (total times this group has been scanned — `0` keeps it in the sweep phase), `leads_total` (lifetime hot+warm+watch leads captured from this group), `leads_recent` (the lead counts from its last 3 scans, newest first — what the plan ranks on once every group has `scans >= 1`), `last_scanned_at` (ISO-8601 timestamp of its most recent scan), `paused_at` (ISO-8601 timestamp of its last pause, blank while active), and `origin` (`discovered | custom | default` — how the source first entered the registry). `tool source-registry plan` stays coverage-first (sweep phase) — never-scanned groups (`scans == 0`), newest `added_at` first — until every one of this client's active groups has at least one scan; only then does it switch to the performance order (`leads_recent` desc → longest-unscanned → member count desc → uid). The plan's own output carries `"phase": "sweep" | "optimize"` and `"unscanned_remaining": <N>` so a run never has to recompute either from the raw group list (`playbooks/10_LEAD_COMPETITOR_DETECTION.md`, "Group sweep before optimisation").
...
- `automation/model_routing_log.jsonl` — append-only, non-PII model-routing audit; one line for every delegated, fallback, or inline-routing decision. Canonical schema: `ts`, `run_id`, `task`, `client_slug`, `operation`, `item_count`, `requested_tier`, `requested_model`, `actual_model`, `execution_ref`, `model_evidence`, `fallback_reason`, `canary_status`, `schema_status`, `row_count_status`, `vocabulary_status`, `batches`, `result_path`. `item_count` is an integer or `unknown` until an unbounded collection is counted. `execution_ref` is the runtime's safe agent/call/task reference; `model_evidence` is `runtime_reported|spawn_request|not_exposed` and `actual_model` is `unverified` when the runtime does not expose it. Never store raw source text, snippets, names, contact data, URLs containing identifiers, credentials, or other PII. `canary_status` is `not_required|pending|pass|fail`; each validation status is `not_required|pass|fail`; `batches` is a count or safe batch identifiers only. `result_path` is the staged file path, never embedded content. Bulk means `item_count > 5` or an unbounded/unknown-size collection that must be exhausted; words such as `batch|list|queue|all|every` only trigger a count/bound check. A bulk extractor pass requires a five-record `pass` canary plus `pass` schema/row-count/vocabulary validation before remaining batches. Each failed extractor batch gets at most one next-tier sub-agent retry with `fallback_reason`; no usable low-tier agent records `fallback_reason: low_tier_subagent_unavailable` and checkpoints, never inline.
...
`status` is `done` at a boundary, `stalled` when a platform's job produced no result within its TTL, `aborted` when the run stopped early (say why in `note`). `platforms` values: `running | done | skipped:{reason} | tripped:{signal} | web_only`. The six `stage`/`stage_index` values, one line per boundary: `find_posts` (1), `find_people` (2), `find_groups` (3), `scan_in_group` (4), `filter_leads` (5), `build_report` (6) — see the Run Progress Rule. `counts.leads_hot`/`leads_warm`/`leads_watch` carry the real, cumulative CRM-captured totals from stage 1 (`find_posts`) onward, never zero-until-`filter_leads` — qualification and capture run per job as each result comes back (`playbooks/10_LEAD_COMPETITOR_DETECTION.md`, "Qualify as you go"), and stage 5 (`filter_leads`) only reconciles them against the CRM.
...
Agents create or refresh this file with the bridge's `tool provider discover --config <client provider_config.local.json> --defaults daily-content-pipeline/provider_defaults.json --out-dir <client integrations/providers folder>`.
