...
- **Freshness gate at write time.** Before a step-1 draft, hooks must be within TTL (else refresh the known URLs). Before every follow-up, micro-refresh the person's 1–2 best sources to (a) find a fresh bump hook and (b) invalidate stale hooks. **Hard rule: a draft may contain only details present in the dossier with an `evidence_url`;** Stage 9 audit checks this mechanically.
