...
- Use one shared Local Collector app/bridge, and one client-specific Chrome extension folder per client Chrome profile/account under `extensions/{client_slug}_extension/`.
...
All agency tools live INSIDE the collector bridge binary as subcommands (the report renderer → `<bridge> tool render-report`, the provider adapter → `<bridge> tool provider`, the shared-scan source registry → `<bridge> tool source-registry`, the keyword-search pool → `<bridge> tool search-pool`; the OutreachCRM module's tools follow the same rule — see its Stage 0). The original Python implementations were retired 2026-07-19; the binary is the ONLY implementation. `<bridge>` is the installed collector bridge executable (the same binary serving `127.0.0.1:17321`, at `{setup-root}/solo-agency-local-collector/bin/collector-bridge-{os}-{arch}`; `.exe` on Windows). Whenever a playbook step names one of these tools, run the binary subcommand (probe: `<bridge> tool render-report` prints usage, exit 2; a binary that fails the probe needs the collector setup re-run). `<bridge>` is ONLY that installed runtime binary, matched to this machine's OS/arch — NEVER run an executable found in the source tree `solo-agency/solo-agency-collector/bridge-go/` (that is Go source; any binary there is a stray dev build, possibly for another OS). Never tell a human to install Python — nothing in Solo Agency needs it.
...
If the human provides no manual private data sources, says they are not sure, or skips the question, the agent must treat that as normal, not as proof that private data sources are unnecessary: discovery of candidate sources from joined groups, subreddits, communities, followed profiles/pages/KOLs, subscribed channels, and feeds needs no separate offer or consent — it runs automatically as part of the first run once the human says yes at step 7. If the human answers "để sau" / not now at step 7, record that status and continue with a clear note that lead/competitor/community coverage is limited until the first run happens.
...
The PDNA Notification question is asked proactively at setup step 8 - value-first framing (instant alerts for hot leads needing fast contact, report-ready, drafts awaiting review, via email + Telegram in one `sendNotification` call), default provider WideCast, API key only, no pressure language and no implied affiliation between Solo Agency and the provider. A decline is honestly recorded as `notification_channel_missing` and re-offered once per later run.

After the first automation report is delivered, the Automation Flow agent may ask about the rest of PDNA:

- Do you want me to set up the rest of PDNA - Production (create real video/blog/social assets), Distribution (publish approved content), and Analytics (measure results) - so approved drafts can become real assets and the system can learn from performance later?
...
If the human wants private data sources before the first automation run, load the private data source playbook, then update the client-specific automation task (the Local Collector itself was already installed at step 4, Kết nối Facebook, Instagram and X, before automation existed). Joined-places discovery needs no separate approval any more: it rides the same yes as step 7's one first-run question (`SOLO_AGENCY_PLAYBOOK.md`, Chạy lượt đầu) — record `approved_pending_first_scan` for its categories, and the first Automation Flow run executes it, Local Collector only, Source Discovery Mode pacing, with results filtered by the Group Potential Rule straight into monitored sources; Setup Flow itself still must not analyze the collected data, generate a report/idea/draft from it, or start daily monitoring — that all happens inside the dispatched run's own session, never this chat. If the human answered "để sau" / not now at step 7, the first Automation Flow run still executes the discovery once it runs on its own schedule, or reports the exact collector blocker if the Local Collector is not healthy by then.
