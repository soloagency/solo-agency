...
The PDNA Notification question is asked proactively at setup step 7 - value-first framing (instant alerts for hot leads needing fast contact, report-ready, drafts awaiting review, via email + Telegram in one `sendNotification` call), default provider WideCast, API key only, no pressure language and no implied affiliation between Solo Agency and the provider. A decline is honestly recorded as `notification_channel_missing` and re-offered once per later run.
