...
3. For each active client, processed in `clients_index.md` order unless `schedule.md` defines a different priority:
   ...
   11C. **Facebook Discovery Pass.** Precondition: `facebook_lead_source: enabled` (Client Intelligence Profile), or `pending` on this client's very first run (Setup Flow already showed the reminder before dispatch). If `skipped`, skip this step entirely and say so plainly in every report this run produces, with the lead-count consequence.
      ...
      - Budget: FIRST RUN ≤ 21 collector calls (3 discovery terms, 3 feed, 3 people, 3 group searches, up to 4 public groups × 3 intent terms), spread ≥ 4 hours, `max_pages` ≤ 4. DAILY ≤ 7 calls (1/1/1/1, up to 2 groups × 2 terms).
