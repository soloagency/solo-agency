...
- Scheduled Daily Runs must run research, private scans if active, analysis, an evidence-rich Idea Matrix and Top 3, HTML report, and notification. They never write full video/blog/social drafts, never run a video writing skill, never call a production provider, and never harvest comment threads. A Daily Run may consume only a completed, fresh Content Evidence Bank snapshot; when it is missing, stale, or unavailable, it must say so in the report and continue with post/source evidence only. Before report HTML/PDF work, scheduled runs must load `playbooks/skills/report-design/SKILL.md` and use `tools/solo_tool render-report` by default instead of writing ad hoc report/PDF scripts. Writing and production remain separate jobs with their own selected-idea evidence, writing gates, provider checks, and approvals.
...
- Time-slot collision rule: before creating or re-timing ANY client automation task, choose a candidate start time: an explicit saved/Boss override when present; otherwise 09:00 local for a fresh setup default. Run `tools/solo_tool schedule-slots --pipeline {setup-root}/daily-content-pipeline suggest --task "{task name}" --client {client_slug} --time "HH:MM" --cadence-hours {H}` (or `--cadence monthly`) with that candidate. With mixed cadences (daily/48h/72h/96h/weekly/monthly) a clock-time compare is meaningless — the tool projects every registered task's future occurrences and pushes the start time in steps (default 15 min) until no occurrence lands in a slot already holding the max (default 10 running tasks). Use the returned `suggested_time`/`suggested_anchor_date` for the native task; when it differs from the candidate, report in ONE sentence why ("09:00 already has 10 tasks running on the days this task fires; scheduled 09:30 instead") — never ask permission for the technical shift. After the native task is created or updated, `... schedule-slots register --task ... --client ... --time ... --cadence-hours ...`; after deleting a task run `remove`, after pausing re-`register --status paused`. The per-slot limit, push step, projection horizon, and the operator's contact email live in `{setup-root}/daily-content-pipeline/system_settings.json`, editable by the operator in the web UI at `/ui/settings` — agents read these settings, never overwrite them.
- **Start time is not finish time.** Wherever a schedule time is automatically selected, explicitly overridden, confirmed, or spoken (the task-creation confirmation, the automation freshness line, the first-run status line), say it as a START time and pair it with the computed finish window from the ETA Rule: "chạy lúc 08:00, dự kiến xong 08:40–09:00". `automation/task_slots.json` `duration_min` for the task is
...
- **The one case where the check cannot run: the bridge/schedule-slots tool is genuinely unavailable.** Step 4 normally installs the bridge before the automation task, so this is an exception, not the normal fresh-install sequence. Do not stall setup or hand-edit `task_slots.json`: create THAT ONE task at 09:00 local, or at the explicit saved/Boss override, and record `slot_check_pending: true` with task name, client, and candidate time in `daily-content-pipeline/automation/automation_manifest.md`. The FIRST action once the bridge is available — before any other `suggest` — is `schedule-slots register` for that task, then clear the flag. This exception covers exactly one task on one install; a second client, a re-timing, or any task created once the bridge exists is bound by the rule above with no exception.
...
- Only AFTER the Boss answers yes to Step 7's run-now question does a setup/config session start `AvenNgo - Solo Agency First Run` itself and say so; it must not generate the report inside the setup chat or start the task before that yes. Only a runtime that cannot start its own tasks instructs the human to run it.
...
6. Load Stage 3 only when a separate writing, production, publishing, provider setup, or notification-provider action is requested. Daily Run report delivery by itself does not load Stage 3.
7. Before a separately requested video provider creation action, load `playbooks/skills/video-script-writing/SKILL.md` and its required modules through the verified client provider when available or repo-local/static fallback when PDNA is missing. Use only the selected idea and completed writing artifact as context. Do not edit/reimplement the skill or send report text unchanged.
...
Scheduled Daily Run completion requires public research, private scans if active, published-URL analytics when published content exists, data analysis, Lead & Competitor Opportunities, an evidence-rich idea matrix, Best Idea and Top 3, Stage 6 plus report-design-rendered HTML/PDF report, notification, and measurement/learning when measurement data exists. It does not require scripts, drafts, provider production-plan queueing, content-plan links, video creation, blog/social creation, or publishing. Those occur only in separate selected-idea writing or production jobs.
...
Qualification and capture are never deferred to stage 5 — every job's compact rows are staged in <=40-row
file-in/file-out batches, classified by one extractor-tier sub-agent per batch (`gpt-5.6-luna` on Codex), validated, and captured before
the next collector job, so `run_progress.jsonl` carries real
...
For any bulk classification (>5 independent records, or a collection whose size is unknown/unbounded and must be exhausted), first run a
five-record extractor canary (Luna on Codex) and validate ids/schema/vocabulary. Append the metadata-only routing audit required by
`daily-content-pipeline/automation/model_routing_log.jsonl` for every canary/pass, without raw text or PII. The main model
may sample/validate output and make CRM/source-registry writes, never reclassify a complete batch. On Codex, each Luna batch failure
permits at most one `gpt-5.6-terra` sub-agent retry with an explicitly logged fallback; other runtimes use their mapped next tier. If no low-tier agent works,
checkpoint/stop `low_tier_subagent_unavailable` rather than inline-classifying.
...
3. For each active client, processed in `clients_index.md` order unless `schedule.md` defines a different priority:
   ...
   11C. **Social Discovery Pass.** One pass covering Facebook, Instagram and X. Precondition: at
      ...
      - Scanning and monitoring a Facebook group need no approval: the extractor-routed Group Potential Rule scores
    ...
    **14b. The feed pass — what the community is talking about.** Then scroll the feed as before, but shallow: its job is no longer to find leads, it is to catch what the terms missed, to see the shift in what the group discusses (which Stage 3 turns into content), and to supply the phrasings that grow the bank. Two or three scrolls is enough for that; the depth budget moved to the search pass. Have the extractor tier (Luna on Codex) classify every post this pass surfaces by the AUTHOR'S TYPE first (`playbooks/LEAD_QUALIFICATION_RULE.md`, Step 1) before deciding whether it is content signal, a lead, or noise — the feed pass has no search terms to anchor on, so the author's own words are the only fit signal it has.
   ...
   26. Build the report's decision payload for every Top 3 and Best Idea: selected `idea_id`, audience pain/question, viewer lesson, non-promotional angle, recommended format, content objective, confidence, freshness, evidence IDs/count, representative excerpts where safe, and reference URLs. Do not write a script, blog draft, caption, storyboard, hook set, or provider payload in this run.
   27. Read only the completed Content Evidence Bank snapshot when it is fresh and available. Do not call comment collection, paginate comment threads, or load raw comment archives in Daily Run. If the snapshot is absent, stale, or unavailable, keep the post/source evidence, label comment evidence accurately, and continue.
   28. Do not load Stage 3, the video writing skill, video-editing skill, provider adapter, or production/publishing operations for Daily Run content work. A separately requested writing or production job owns those gates and may act only on a selected idea/evidence bundle with the required approvals.
   ...
   28d. **Orders pass.** Load `playbooks/ORDER_RULE.md` §4–5 and every `orders/*.md` brief of this client with `status: active`; for each whose cadence is due (its own line; default this run), LOAD LEDGER the brief, execute its "How I do it" with the hands it names, route every write to its approval point (never publish from the run), append its run-log row with the evidence read, and carry its output line into the report's **Orders** section and the run's notification line. A brief with `status: waiting_boss` is listed, not run; a brief that could not run gets a run-log row saying why. Bulk inside an order follows step 3G's sub-agent contract.
...
If this client's WideCast notification/Telegram/email fallback is connected and WideCast report upload supports HTML, the operator notification link should use the uploaded report URL, not only a local file path. Daily Run does not mint a content-plan link or queue ideas for production. If upload fails, discovery fails, auth fails, account identity cannot be proven client-scoped, or the current provider spec does not support HTML upload, log the blocker and send the best available HTML path/link plus PDF companion path/status plus INTERNAL_REPORT path/status.
...
- During one-time setup, after the profile and source plan are known and before the client-specific automation task is marked ready, automatically configure a fresh client with no explicit saved or Boss-provided schedule as daily at 09:00 local (all days). Call `tool schedule-slots suggest` with 09:00, create the native task at the returned actual time, then `register` it; report that actual start time and the computed finish window. Do not ask a cadence/start-time question or use an `**[ACTION REQUIRED]**` block for this reversible default; the Boss can ask Sam to change it later. An explicit cadence/time always wins; repair/update preserves an existing schedule. Manual, multiple-times-daily, and weekly remain supported when expressly requested.
- Never create a native task at an unchecked time, with the single exception named in that rule: if the bridge/schedule-slots tool is genuinely unavailable, create the first task at 09:00 local (or the explicit override), record `slot_check_pending`, and register it the moment the bridge arrives.
