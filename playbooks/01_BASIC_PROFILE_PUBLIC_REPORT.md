...
- Step 5 automatically configures default sources and per-channel keyword banks before schedule/automation setup. It asks no custom-source question; step 7 asks only the first-run yes/no question, then resync the task if source state changes.
- The Local Collector bridge and this client's extension are installed at step 4 (Kết nối Facebook, Instagram and X), before the automation task exists. If a custom source is voluntarily supplied during setup and the bridge/extension is not healthy, clearly mark it pending and resync the automation run contract.
...
- If the human asks to run, create, generate, show, refresh, or update a report while this stage is being used for Setup Flow, do not run the report in the setup chat. Finish or resync the contract and, only after Step 7 yes, autonomously create/start the separate First Run in its own session. Only a genuinely incapable OTHER runtime may name it for human start; Codex/Claude use one-time/recovery methods and never ask for a scheduler click.
...
- The first report must run in Automation Flow through the separate one-time `{Client} - Solo Agency First Run`; the recurring Daily Run is not the First Run task.
- If the human asks to run, create, generate, show, refresh, or update a report while still in Setup Flow, verify/resync the contract and follow Step 7: after yes, Sam autonomously starts the separate First Run and reports back. Only genuinely incapable OTHER runtimes may name that First Run for human start; Codex/Claude recover/escalate rather than hand off the scheduler. Do not ask the Boss to operate a scheduler; Step 7 permission remains the only run question.
...
4. **Kết nối Facebook, Instagram and X**, right after the inference above. Open with one plain-language value sentence (for example: "Bước này để em quét được Facebook, Instagram và X: feed, người, bài viết và bình luận public — đó là nơi phần lớn lead đầu tiên sẽ đến từ."), then install the Local Collector bridge and this client's Chrome extension — the same extension covers all three platforms — per the local/remote rule (`playbooks/08_LOCAL_COLLECTOR_TECHNICAL_PROTOCOL.md`, `playbooks/SETUP_FLOW_ENTRYPOINT.md` "Kết nối Facebook, Instagram and X (step 4)"): on a local runtime the agent writes `setup_collector.sh`/`.ps1`, gives the one plain-language safety line, asks for consent once, runs it itself, and waits for `GET http://127.0.0.1:17321/status` (up to 60 seconds); on a remote runtime that cannot see the install root or verify `/status` in time, it hands the human the one-line command instead. Once `/status` succeeds and proves the current workspace, automatically print and show `http://127.0.0.1:17321/ui/{client_slug}#revenue-engine` under the SHOW RULE, then make `/ui/{client_slug}/extension` the foreground tab for the two-gesture extension install (reveal the folder, open `chrome://extensions/`; the human turns on Developer mode once and drags the folder in). Keep Revenue Engine behind that active Extension tab while waiting; only after `extension_health` is recent bring Revenue Engine forward again. This is localhost dashboard display only, never private-source browsing and never an `**[ACTION REQUIRED]**` block. Immediately after the bridge answers `/status`, deliver the Login Reminder verbatim (it names Facebook, Instagram, and X) and record `facebook_lead_source: enabled|web_only|pending`, with `instagram_lead_source` and `x_lead_source` set alongside it automatically — the three platforms are default sources, enabled automatically as the extension checks in, no extra question per platform — `web_only` only on a clear confirming phrase (e.g. "không dùng Facebook"), never "để sau"/silence, which leave it `pending`. If the human is still deciding, do not stall — continue to step 5 with `pending`; the client's first dispatched run is gated only on `facebook_lead_source` being `enabled` or `web_only`, never `pending` — Instagram and X follow automatically (full contract, the 90-second extension help loop, and the acknowledgment script: `playbooks/SETUP_FLOW_ENTRYPOINT.md`, "Kết nối Facebook, Instagram and X (step 4)"). The FIRST RUN discovery budget then applies to whichever run turns out to be this client's first-ever Social Discovery Pass (`facebook_discovery_first_pass_done`), not necessarily this client's very first automation run.
5. **Sources.** Automatically configure the default sources this client gets without doing anything — Google search driven by the keyword bank, Facebook, Instagram and X through the connected extension when available, and industry sites — then build the keyword bank per channel FROM `buyer_profile.types` — the channel table, generation procedure, and quality gate in `playbooks/00_CORE_CONTEXT_REQUIREMENTS.md`, "Buyer Profile Channel Keyword Table" — not from intent phrases alone. Do not ask for custom sources or create an `**[ACTION REQUIRED]**` block. Preserve any URL the Boss volunteers with `origin: custom` in `collector_config.json` and process it when the collector is ready; it cannot delay schedule setup or first-run dispatch. After setup, add custom sources through Revenue Engine `watch_source` ("Watch a group, page, profile, or competitor") or a direct request to Sam, followed by Automation Resync. The bridge already exists since step 4, so write each channel's bank through its own tool (`public-keywords` for the open web, `source-keywords` for in-group), and the plain staging file is only the fallback when the bridge is unreachable. Show only a compact `Pain-Point Keyword Sample`, not a generic industry keyword dump. Then show the complete setup summary, ask the human to correct only what is wrong, and save the Client Intelligence Profile after that correction chance.
6. Configure the schedule/routine before the first agency run, using the best scheduling mechanism available in the environment, and create or verify the client-specific automation task. On a fresh setup with no explicit saved or Boss-provided schedule, automatically set daily 09:00 local: call `tool schedule-slots suggest` with 09:00, create at the returned actual time, then `register`; report the actual start time and finish-window estimate, and say the Boss may ask Sam to change cadence/time later. Do not ask a cadence/start-time question or use an `**[ACTION REQUIRED]**` block for this reversible default. An explicit schedule wins; repair/update preserves the existing schedule. If the bridge/schedule-slots tool is genuinely unavailable, create at 09:00 local (or explicit override), set `slot_check_pending: true`, and register once available. Do not stall setup here. Configure the initial task with the default sources plus any custom sources voluntarily supplied during setup; mark one pending only if the bridge/extension is not healthy yet, and resync the task once it is. On a local Claude Code runtime, right after this task is created, ask the one unattended-permissions consent and, on yes, write the user-level allow rules (`playbooks/04_DAILY_SCHEDULE.md`), then proceed to Step 7's separate permission gate.
...
7. **Chạy lượt đầu (First run).** After the recurring Daily Run exists, ask exactly one question — may I run the first report now — and only on yes create/start the separate one-time `{Client} - Solo Agency First Run`, wait/report/clean it up (`playbooks/SETUP_FLOW_ENTRYPOINT.md`, "Step 7 — Chạy lượt đầu"); record lifecycle. Never ask the Boss to operate a scheduler on Codex/Claude, and never reuse the Daily Run. Step 5 was automatic; no custom-source answer is needed here.
...
Note: step 5 does not ask about custom sources; do not introduce a custom-source question in this follow-up.
...
- Target location only if location matters and cannot be inferred.
- If the human volunteers custom sources early, preserve them as `pending_private_review` and process them when the collector is ready (do not ask for them here).
...
5. **Sources.** Automatically set up the default sources (Google search with the keyword bank, Facebook, Instagram and X through the connected extension, and this client's industry sites) and save the source plan and keyword bank. Do not ask for custom sources or add a required action; preserve any URL volunteered during setup and process it when the collector is ready.
6. Configure the recurring schedule/routine once the basic source plan is known, prepare or verify the client-specific extension folder under `extensions/{client_slug}_extension/`, and create/resync the exact `Nguyen Law - Solo Agency Daily Run`. With no explicit saved or Boss-provided schedule, automatically use daily 09:00 local through Stage 4's Time-slot collision rule (`tool schedule-slots suggest` → create at returned actual time → `register`), report the actual time/finish window, and say it can be changed later. An explicit schedule wins; repair/update preserves one. If the tool is genuinely unavailable, create at 09:00 local (or explicit override), record `slot_check_pending: true`, and register once the bridge exists. Configure the initial task with default sources and any active custom source voluntarily supplied during setup; no custom-source response gates it.
7. **Chạy lượt đầu (First run).** After Daily Run exists, ask exactly one question — may I run the first report now — and only on yes create/start the separate `Nguyen Law - Solo Agency First Run`, wait/report/clean it up (`playbooks/SETUP_FLOW_ENTRYPOINT.md`, "Step 7 — Chạy lượt đầu"); never reuse Daily Run or ask Codex/Claude Bosses to operate a scheduler. Step 5 was automatic and no custom-source response is needed; discovery needs no approval — the run judges every readable group with the Group Potential Rule and monitors the good ones. The Local Collector bridge and extension were already installed at step 4.
...
The agent must configure the routine, prepare the `Nguyen Law - ...` automation task, then ask step 7's one question — may I run the first report now — and dispatch on yes (step 5 was already automatically complete). Setup Flow must not run Nguyen Law's report directly.
...
- Collector setup is no longer gated on custom sources — it already happened at setup step 4 (Kết nối Facebook, Instagram and X), before the automation task and automatic step 5. This section covers what step 5 (or a later run) does when it needs to check or repair that install, not a fresh first install.
...
- During Setup Flow, do not run a report. Finish by preparing/resyncing the recurring Daily Run, then ask Step 7's one run-now question and only on yes autonomously create/start the separate First Run; Daily stays untouched and Codex/Claude never ask for a scheduler click.
- Step 5 automatically configures default sources and keyword banks before schedule/routine setup and automation task creation. The Local Collector was already installed at step 4; if a custom source was voluntarily supplied but the bridge/extension is not healthy, clearly mark it as `pending_private_activation` in the automation contract so the first automation run can continue with default sources if needed, then resync the task once it is healthy.
