...
- active goal;
- active campaign;
- run-window published content status;
- run-window and 7-day metrics;
...
### Handoff → capability agent (2026-09-10): facebook_logged_in

Owner decision 2026-09-10 (browser + profile handling for "Kết nối Facebook") only covers picking
the right browser/profile and opening the extensions page for install. It explicitly defers one
piece to a future capability agent, not this task: telling connected-but-wrong-profile apart from
connected-and-logged-in. This entry is that handoff.

What to build:

- On each check-in, the extension should detect whether the **current profile** is logged into
  Facebook. No page scraping needed — either:
  - fetch `https://www.facebook.com/me/` with credentials (`credentials: "include"`) and inspect
    the final URL after redirects (logged-out lands on a login/checkpoint URL, logged-in redirects
    to the profile), or
  - a lightweight GraphQL/viewer-id check (the same interception path `gql_intercept.js` /
    `platforms/facebook/` already taps can read the current session's viewer id when present).
- Report the result to the bridge as a new field on check-in, exact name:
  **`facebook_logged_in`**, one of three string states: `"true"`, `"false"`, `"unknown"`
  (`"unknown"` covers request failure, timeout, or not-yet-checked — never invent a fourth state,
  and never omit the field once this ships).
- The bridge should surface this per-client in `/status` under `extension_health` (same place the
  90-second diagnostics already read from) and on the extension page in the dashboard, next to the
  existing connected/not-connected indicator.
- The agent then uses the three-way signal to distinguish:
  - connected + `facebook_logged_in: "true"` → connected and logged in, proceed normally;
  - connected + `facebook_logged_in: "false"` → connected but this profile is not logged into
    Facebook (wrong profile, or logged out) — prompt the human to log in on that profile, don't
    silently retry;
  - not connected at all → extension never checked in, unrelated to this field;
  - `facebook_logged_in: "unknown"` → treat like not-yet-known, re-check next poll, do not alarm
    the human on a single unknown.
- Manifest permission: verified against `solo-agency-collector/chrome-extension/manifest.json` —
  `host_permissions` already includes `"https://*/*"` and `"http://*/*"` (broad, not scoped to
  `facebook.com` specifically), which already covers a `fetch()` to `facebook.com` from
  `background.js`. **No manifest change is needed** to add this check; confirm this still holds
  if `host_permissions` is ever narrowed later.
...
3. Ask Codex to read the log and review:
   ...
   - where it failed to measure run-window / 7-day results;
