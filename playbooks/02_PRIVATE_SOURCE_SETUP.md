...
11. Save approved sources to `private_data_sources`, and register each one in the cross-client source registry: `tools/solo_tool source-registry --pipeline {setup-root}/daily-content-pipeline register --client {client_slug} --url U --platform P --source-type T --kind private --cadence C --priority P2`. Scope rule: a source that IS the client's own asset (their own page, group, channel, or website) gets `--scope exclusive` (never shared with other clients); every third-party source (industry groups, competitor pages, communities) stays `shared` so clients monitoring the same source reuse one scan. Register each Facebook group under ONE canonical URL form (prefer the group page's vanity URL) — a numeric-ID URL and a vanity URL of the same group do not merge automatically.
