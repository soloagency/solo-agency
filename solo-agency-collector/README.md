...
- Pro-only capabilities are marked `tier: pro` / `feature` in `collector_capabilities.json`; the bridge reads tiers from the catalog compiled into the binary only, and the extension re-verifies the same token (`solo_entitlement.js`) before running them.
