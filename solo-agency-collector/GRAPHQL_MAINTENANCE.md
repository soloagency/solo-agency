...
- `privacy` / `member_count` (2026-09-09): the node carries NO numeric member field and NO privacy enum (`_discover.deep` on `/search/groups/`). `privacy` comes FIRST from `view_model.ctas.primary[i].profile.viewer_join_state` — `CAN_JOIN` = public, `CAN_REQUEST` = private, locale-independent; live 2026-09-09 on 25 "realtor" results it agreed with the rendered line 25/25 — then from the card's descriptor line `view_model.primary_snippet_text_with_entities.text` matched against a multi-language word list (`privacy_source` says which). `member_count` is parsed by `parseGroupSnippet`/`parseGroupNumber` from that line without needing a "members" word: exact magnitude table (K, N, tsd, mil, тыс, พัน, 万, 만, …) or a grouped number; a short unknown token (Turkish "B") leaves it null. The raw line ships as `snippet`, its UI locale as `snippet_lang`, the winning segment as `member_count_text`.
