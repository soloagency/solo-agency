...
Live: `"Post 5 post 5 post 5"` → `/posts/000000000000001/`, commented, `done`/`verified`.
...
| Bridge (Go) | `bridge-go/` |
| Deploy script | `/path/to/solo-agency/deploy-soloagency.sh` |
| Test client build (aven-ngo) | `/path/to/dev-install/extensions/aven-ngo` |
| Live production clone (codex) | `/path/to/live-install/solo-agency` |
| Collector output (inbox) | `/path/to/live-install/daily-content-pipeline/collector/inbox` |

`aven-ngo` is only an EXTENSION NAME — the Facebook account behind it is the operator's
own (`example.operator`). That is why it can act on the operator's own private test targets.
...
```
Verified live with a vanity (`example.recipient.000000`): the "Continue" gate appears, and
...
1. Read this file, then `git -C /path/to/solo-agency log --oneline -5`
...
5. Test targets the user has authorised for write actions (private, theirs — do NOT use
   any other target):
   - post permalink: `facebook.com/example.operator/posts/pfbidEXAMPLE`
   - reel: `facebook.com/reel/000000000000000/`
   - group: `facebook.com/groups/000000000000000` (contains posts "Post 5 post 5 post 5"
     → `…/posts/000000000000001/`, "Post 3: …" → `…/posts/000000000000002/`, "Post 2")
   - DM thread: `messenger.com/e2ee/t/example.recipient.000000` (recipient name "Bob Nguyen")
