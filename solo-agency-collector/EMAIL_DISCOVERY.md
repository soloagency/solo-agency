...
| Input | Meaning |
|---|---|
| `profile_url` | optional; defaults to the page the job opened. Vanity (`/sample.business`) or numeric (`profile.php?id=61581802543283`). A URL that already carries a tab (`…&sk=reels_tab`, `/about`) is trimmed back to the profile base automatically. |
...
```json
{ "profile_url": "https://www.facebook.com/sample.business",
...
|---|---|---|---|---|---|
| `sample.business` (`info@example.com`) | ✅ | ✅ | ✅ | ✅ | ✅ |
| `sample.lead` (`sample.lead@example.com`) | ✅ *(only after "See more" was expanded)* | ✅ | — | — | — |

Before "See more" expansion shipped, `sample.lead` returned `emails: []` on the main page,
