...
async function run() {
  ...
  console.log("\n== contact data is collected across tabs, not just the first hit ==");
  {
    ...
    const res = await ctx.window.__soloGqlPaginate("fb.profile.dossier", FAST);
    const it = (res.items || [])[0] || {};
    check("the published address was found", (it.emails || []).indexOf("advisor@northstar-wealth.invalid") !== -1, it.emails);
  ...
  console.log("\n== entering at /about does not re-enter About, and does not re-read it ==");
  {
    ...
    const pages = { main: page(["Ann Vuong", "Real Estate Agent", "contact@sample-house.invalid"]),
                    work: page(["Owner/President at Reach Home Loans"]),
                    education: page(["Went to Juanita High School"]),
                    contact_info: page(["contact@sample-house.invalid"]), intro: page(["Real Estate Agent"]) };
    const { ctx } = makeCtx({ pages, slugs: SLUGS, offers: Object.keys(SLUGS), seeMore: 0 });
    ctx.location.href = "https://www.facebook.com/SampleHouse/about";
    const res = await ctx.window.__soloGqlPaginate("fb.profile.dossier", FAST);
    const it = (res.items || [])[0] || {};
    ...
    check("the address came from the overview anyway", (it.emails || []).indexOf("contact@sample-house.invalid") !== -1, it.emails);
  ...
  console.log("\n== sections come from GraphQL, and the DOM is never touched ==");
  {
    ...
    served[tok("B")] = { data: { section: { rows: ["Email", "contact@sample-house.invalid"] } } };
    ...
    check("the address arrived", (it.emails || []).indexOf("contact@sample-house.invalid") !== -1, it.emails);
  ...
  console.log("\n== a Page's address becomes location, and a review never becomes the bio ==");
  {
    ...
    const pages = {
      ...
      contact_info: page(["Address", "30 Eglinton West, Mississauga, ON, Canada, L5R 3E7", "Phone", "+1 555-555-0144"]),
    ...
    check("the phone was read from its label", (it.phones || []).some((p) => /555-555-0144/.test(p)), it.phones);
