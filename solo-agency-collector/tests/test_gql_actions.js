...
(async () => {
  ...
  const domPost = (url, text) => ({ url, text });
  {
    const G = "https://www.facebook.com/groups/000000000000000/posts/";
    const ctx = makeCtx({});
    ctx.window.__soloGqlPaginate = async () => ({ available: true, items: [post("Post 2: Post 2", G + "668676911902646/"), post("Post 3: Post 3", G + "000000000000002/")] });
    const r = await ctx.window.__soloActResolve("fb.post.comment", { match_text: "Post 5 post 5 post 5", _target_url: "https://www.facebook.com/groups/000000000000000",
      _dom_posts: [domPost("/groups/000000000000000/posts/000000000000001/?__cft__[0]=AZX", "Post 5 post 5 post 5")] });
    const it = r.items[0];
    check("the newest post, invisible to GraphQL, is now found", r.status === "resolved", r.status);
    check("resolved to the right permalink", it.matched_post && /000000000000001/.test(it.matched_post.url), it.matched_post);
