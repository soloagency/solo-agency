...
// 2026-08-16 (directory page los-angeles-ca with ?name=kim; team-lead profile "Sample Realty
// Properties"; team-member profile "sampleagent") — the same field names, nesting and value
...
const SRC = fs.readFileSync(path.join(__dirname, "..", "chrome-extension", "platforms", "zillow", "zillow_extract.js"), "utf8");
...
// Team-lead profile (Sample Realty) — values as observed.
...
(async function main() {
  ...
  console.log("zillow.agents.list — __NEXT_DATA__ missing: DOM fallback from /profile/ anchors, available stays true");
  {
    ...
    check("dom card carries name/screen_name/rating/reviews/is_team", res.items[0].name === "Jordan Lee" && res.items[0].rating === 4.9 && res.items[0].reviews_count === 28 && res.items[1].is_team === true, res.items);
  ...
  console.log("zillow.profile.enrich — team-lead profile (Sample Realty)");
  {
    const ctx = makeCtx({
      href: "https://www.zillow.com/profile/Sample%20Realty",
      title: "Alex Rivera - Real Estate Agent in Venice, CA - Reviews | Zillow",
      nextData: profileNextData("lead"),
      nodes: [el("h1", {}, "Alex Rivera"), el("a", { href: "tel:5555550142" }, "(555) 555-0142")],
    });
    const res = await ctx.window.__soloZillowRun("zillow.profile.enrich", {});
    ...
    const r = res.items[0];
    check("profile_url rebuilt from screenName (space -> %20), name", r.profile_url === "https://www.zillow.com/profile/Sample%20Realty" && r.name === "Alex Rivera", [r.profile_url, r.name]);
    ...
    check("intro_bio: HTML stripped, entity decoded, paragraphs kept", r.intro_bio === "Alex Rivera is the founder & CEO of Sample Realty.\nSince 2004 the team has closed over $8B.", r.intro_bio);
    check("about_lines carry brokerage, licence, sales stats, service areas, team", ["Brokerage: Sample Realty", "Real Estate Licenses: 01858429 (CA, disabled, exp 2029-04-02), 01421451 (CA, active, exp 2028-03-16)", "Sales last 12 months: 266", "Total sales: 3,339", "Price range: $10K-$18M", "Average price: $1.9M", "Sales numbers represent all team members", "Team: lead of Sample Realty (2 members)", "Top Agent on Zillow", "Years in industry: 22", "Member since: 12/08/2006"].every((l) => r.about_lines.indexOf(l) !== -1) && r.about_lines.some((l) => /^Service areas \(3\): Beverly Hills, CA; Los Angeles, CA; Malibu, CA$/.test(l)), r.about_lines);
    ...
    const z = r.zillow;
    check("zillow.team: role lead, 2 members with profile urls", z.team.role === "lead" && z.team.team_name === "Sample Realty" && z.team.member_count === 2 && z.team.members[0].profile_url === "https://www.zillow.com/profile/sampleagent" && z.team.members[1].profile_url === "https://www.zillow.com/profile/Kerry%20Ann%20Sullivan" && z.team.members[0].reviews_count === 144, z.team);
    ...
    check("zillow.rating / listings / specialties / service areas / socials", z.rating.average === 5 && z.rating.count === 1664 && z.listings.for_sale === 58 && z.listings.for_rent === 16 && z.specialties.length === 4 && z.service_area_count === 3 && z.socials.facebook === "https://www.facebook.com/samplerealty" && z.socials.x === "https://twitter.com/sample_realty" && z.socials.linkedin === "http://www.linkedin.com/in/samplealex", z);
  ...
  console.log("zillow.profile.enrich — team-member profile (sampleagent): title null, member-of-team, no team disclaimer");
  {
    ...
    check("website = personal team page; socials only instagram", r.website === "https://example.com/team/sample-agent/" && Object.keys(r.zillow.socials).join(",") === "instagram", [r.website, r.zillow.socials]);
