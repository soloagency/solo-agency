...
|---|---|---|---|---|---|
| feed.home | the logged-in home feed (`fb.newsfeed`) | built | feasible — `PolarisFeedTimelineRootV2Query` seen live on `/` | feasible — HomeTimeline / HomeLatestTimeline ops | feasible — `/feed` |
| search.posts | keyword search over all public posts (`fb.search.posts`) | built (top / posts) | **built** `ig.search.posts` — top grid only, web has no "latest" tab | feasible — `?f=top` and `?f=live` (latest) plus advanced operators (`from:`, `since:`, `min_faves:`); richer than Facebook | feasible — content tab, sort relevance/date, date filter |
| search.people | keyword search returning people (`fb.people.search`) | built | **built** `ig.people.search` — the search box's top matches (about 5–30), no pager | feasible — `?f=user` | feasible — people tab with location/company/title filters |
...
| profile.header | name / handle / basic header (`fb.profile.header`) | built | **built** (inside `ig.profile.enrich`) | feasible — UserByScreenName | feasible — top card |
...
| profile.enrich | bio, category, location, links, counters (`fb.profile.enrich`) | built | **built** `ig.profile.enrich` — bio, category, bio links, business address, counters, is_business/is_verified | feasible — bio, location, website, joined, counters, professional category | feasible — headline, about, location, counters |
...
| profile.posts | posts of a profile with author + engagement (`fb.profile.posts`) | built | **built** `ig.profile.posts` | feasible — UserTweets / UserTweetsAndReplies | feasible — `/in/<x>/recent-activity/all/` |
...
| post.comments | comments of one post with author identity (`fb.post.comments`) | built | **built** `ig.post.comments` — owner may hide comments (reason `comments_hidden`) | feasible — TweetDetail conversation | feasible — comments panel, "load more" |
| post.react | like / react (`fb.post.react`, write) | built | feasible — `POST /web/likes/<media_id>/like/` with csrftoken | feasible — FavoriteTweet mutation | risky — reactions are the first thing LinkedIn's anti-automation watches |
