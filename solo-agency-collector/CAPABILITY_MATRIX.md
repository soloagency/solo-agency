...
| message.send | direct message to a person (`fb.message.send`, write) | built | **built** `ig.message.send` — drives the profile's Message button + thread composer; to a non-follower it lands in Requests | **built** `x.dm.send` — the profile's Message control + conversation composer; `dm_not_allowed` when the account's DMs are closed | risky — 1st-degree only; InMail is paid and capped |
