...
formats. (On a LOCAL runtime — the agent's own shell IS the human's machine, e.g. Claude Code
desktop/CLI or Codex CLI — the agent runs this script itself: one plain-language safety line,
one consent ask, then execute, then poll `/status` for up to 60 seconds. This is safe because
the script hands the running bridge to an OS-level autostart supervisor — see "Autostart at
boot" below — so it is not a child of the agent's own shell/turn. On a REMOTE runtime, a hosted
sandbox where `127.0.0.1:17321` is not the human's machine — detect this by the agent being
unable to see the install root on its own filesystem, or `/status` still failing 60 seconds
after a bootstrap attempt — the human still runs it instead, but runs ONLY this one script; no
extra bridge-start command.)
...
5. Extract/copy the Chrome extension template into a per-client absolute `extensions/{client_slug}_extension/` folder.
6. Select the correct bridge binary for the user's OS/CPU.
7. Same Local vs remote runtime rule as the canonical script above: on a LOCAL runtime the agent runs the downloaded executable/setup script itself (one safety line, one consent ask); on a REMOTE runtime it prepares the files and gives the human the one-line command to run outside its sandbox instead.
8. Prefer persistent scheduler mode for unattended private data source collection.
9. On a remote runtime, use on-demand mode only when a human-run process or OS startup service already has the bridge reachable for that run.

The user still needs to do the two gestures a person still has to do (Developer mode, drag the folder) to install the extension once — see the two-gesture install below — until the extension is available in a browser's extension store.
...
solo-agency-local-collector/
  ...
  collector.log
extensions/
  {client_slug}_extension/           # legacy installs may still have {client_slug}/
    manifest.json
    background.js
    popup.html
    popup.js
...
Folder resolution order (OWNER DECISIONS 2026-09-10 item A): the bridge's `uiExtensionInfo` picks a client's extension folder as — an `extension_registry.json` pin, then `extensions/{client_slug}_extension/`, then, only for installs made before the 2026-09-10 rename, the legacy `extensions/{client_slug}/`. Always create the `_extension` folder for a new client; the legacy name is read-only backward compatibility.
...
If `extensions/{client_slug}_extension/manifest.json` does not exist, prepare it from the local extension template with the bridge binary (Go, every OS, needs no Python or Node):

```bash
<bridge> tool extension prepare --client-name "{Client Name}" --client-slug "{client_slug}" --instance-id "{extension_instance_id}" --root "{ABSOLUTE_AGENCY_ROOT}"
```

`solo-agency-collector/scripts/prepare_client_extension.sh "{Client Name}" "{client_slug}" "{extension_instance_id}" "{ABSOLUTE_AGENCY_ROOT}"` is a thin wrapper around that command. Only if the bridge binary is not on the machine yet: copy `solo-agency-collector/chrome-extension/` to `extensions/{client_slug}_extension/`, patch that client's `manifest.json` so the extension name starts with the client name, and create `client_binding.json`.

**Two-gesture install.** On a LOCAL runtime, trigger it yourself: call `POST http://127.0.0.1:17321/api/ui/{client_slug}/install-extension` with no `browser`/`profile_directory` fields — the bridge reveals the client folder highlighted inside extensions/ (macOS `open -R`, Windows `explorer /select`) and best-effort opens a Chromium extensions page in whichever browser is already running, brought to the front, in the same action (it can shell out directly since it runs on the human's own machine: `open -a "Google Chrome" "chrome://extensions/"` on macOS, `cmd /c start "" chrome "chrome://extensions/"` on Windows, `google-chrome`/`xdg-open` fallback on Linux). Then poll `GET /status` until `extension_health.status` is recent (75-second grace window) after the human does the two gestures: open their own browser's extensions page if it did not already come to the front, turn on Developer mode, and drag the highlighted folder onto it. The `/ui/{client_slug}/extension` dashboard page shows a short recorded video right above those two steps by default — mention it to the human ("the page has a 30-second video showing exactly this" — in the human's own language). On a local runtime the video is the local file on disk (`{ABSOLUTE_AGENCY_ROOT}/solo-agency/solo-agency-collector/bridge-go/assets/setup_extension_chrome_small.mp4`, Edge: `..._edge_small.mp4`) — send it into chat when the runtime can, otherwise open it with the OS default player and/or the dashboard page; the public GitHub links are for a remote runtime only, never printed here. The bridge also serves both recordings directly at `http://127.0.0.1:17321/ui/assets/help/setup_extension_chrome_small.mp4` (Edge: `…_edge_small.mp4`) — embedded in the binary, so that URL works no matter how old the human's clone is and is the preferred local source; the on-disk copy under `solo-agency-collector/bridge-go/assets/` only exists on installs that have pulled since it was added, so check the file exists before naming or opening it, and fall back to the embedded URL when it is missing.

**No browser question.** Setup never asks about the browser or the profile the human uses — most humans do not know what a profile is, and the extension is bound to the client by `client_binding.json`, never by a profile. `GET http://127.0.0.1:17321/api/ui/{client_slug}/browsers` exists only for the advanced (`?advanced=1`) install page; the agent never calls it to decide whether to ask something, because it never asks. The human installs the extension in whatever Chromium-based browser they already use — Chrome, Edge, Brave, Vivaldi, Opera, Chromium — and the agent never says which ones it detected and never asks a Chrome-vs-Edge question or anything like it. Safari/Firefox are not supported (tell the human to install Chrome instead) — per "Browser: never asked, never named" in `playbooks/SETUP_FLOW_ENTRYPOINT.md`, "Kết nối Facebook, Instagram and X (step 4)". The install page's own account dropdown, when the browser has 2+ profiles, lives only behind `?advanced=1` for an operator; nothing about the browser or an account is ever asked in chat, for any client.

**If check-in stalls — 90-second help loop.** If `extension_health` has not gone recent within 90 seconds of triggering the install action, do not just repeat the reminder. Diagnose in this order and say ONE short line naming the likely cause: (1) Developer mode is still off — the toggle top-right on the extensions page; (2) the folder was dropped into a window that is not the one signed into Facebook; (3) the browser was closed or the page was navigated away. Re-trigger the install action (`POST /api/ui/{client_slug}/install-extension`) so Finder and the extensions page are back in front, then wait again. Run this diagnose-and-retrigger cycle at most 3 rounds. If the human says they cannot find the Developer mode toggle, that is a managed (work) browser — say so plainly, suggest trying from a personal (non-work) computer instead, and only then offer the web-only escape (`playbooks/SETUP_FLOW_ENTRYPOINT.md`, "Kết nối Facebook, Instagram and X (step 4)"). Never offer that escape before this help has been given, and never in the first message.

On a REMOTE runtime, tell the user with the resolved absolute path, not a relative path:

```md
Please install the local collector extension for {Client Name}.

**Easiest — the page shows everything you need:** open `http://127.0.0.1:17321/ui/{client_slug}/extension`. It plays a 30-second install video and shows the extension folder's absolute path (with a Copy button) above the two steps. Open your own browser's extensions page, turn on `Developer mode`, then drag that folder onto the extensions page. It turns green ✓ when it connects. If the video doesn't play: https://github.com/soloagency/solo-agency/blob/main/solo-agency-collector/bridge-go/assets/setup_extension_chrome_small.mp4 (Chrome) or https://github.com/soloagency/solo-agency/blob/main/solo-agency-collector/bridge-go/assets/setup_extension_edge_small.mp4 (Edge).

**Manual fallback:**
1. Load it into whichever browser window you already use for Facebook for {Client Name}. Any Chromium-based browser works — Chrome, Edge, Brave, Vivaldi, Opera, Chromium; if only Safari or Firefox is installed, install Chrome first — it's free, and using it just for Facebook is fine.
2. Go to `chrome://extensions` (or the equivalent page for the browser you're using — `edge://extensions`, `brave://extensions`, etc.).
3. Turn on `Developer mode`.
4. Click `Load unpacked`.
5. Select this folder:
   `/ABSOLUTE/PATH/TO/extensions/{client_slug}_extension/`

Important: if you also see `/ABSOLUTE/PATH/TO/solo-agency/solo-agency-collector/chrome-extension/`, do not select it. That is the toolkit/source copy. Select only the client folder under `extensions/{client_slug}_extension/`.

After this one-time setup, keep that browser window logged in to the private data sources approved for {Client Name}. The shared Local Collector app/bridge handles routing and local file output.
```

Create a ready-to-run setup file such as:
...
On a LOCAL runtime, run that file yourself (one safety line, one consent ask, then execute). On a REMOTE runtime, give the user exactly one short command:
...
- **Autostart VERIFY duty (capability ladder).** Every setup run writes the outcome to
  ...
  predates autostart scripts; (3) an agent that can do neither (a REMOTE runtime) hands the human
  the ONE copy-paste command for their OS from rung 1 and asks for the pasted output. The FIX is
  always the same script — re-run the setup script, never hand-craft a custom service, never
  bootout/register a service piecemeal — and on a LOCAL runtime (rungs 1-2) the agent re-runs
  that script itself (one consent ask), never a REMOTE runtime (rung 3), which still hands it to
  the human. Sandboxed/remote agents must not attempt the step-0 bootout/stop commands above
  either — those are for a local runtime's own shell; the setup script performs the same detach
  itself when it runs, whoever runs it.
...
```json
{
  ...
  "sources": [
    {
      "name": "zillow profile sampleagent",
      "url": "https://www.zillow.com/profile/sampleagent",
      "source_type": "public",
      "platform": "zillow",
...
- on a LOCAL runtime, re-run the current setup's script yourself (one consent ask) so it can stop the old `collector-bridge` process and restart with the current config/output paths; on a REMOTE runtime, give the human the one-line Local Collector setup/start command to run outside its sandbox to do the same;
- remind the human that one machine should have one active shared Solo Agency Local Collector runtime, and one client-specific extension per client Chrome profile/account loaded from `extensions/{client_slug}_extension/`. If stale extension entries exist, the human should remove or disable only the stale entries in `chrome://extensions`.
...
If the bridge is offline: on a LOCAL runtime, run the setup/start script yourself (one consent ask) and re-check; on a REMOTE runtime, give the human the exact absolute-path start command to run outside its sandbox. If the extension is stale or missing, continue with public data sources, skip private data sources for this run, and notify the human through WideCast MCP / Telegram when available.
