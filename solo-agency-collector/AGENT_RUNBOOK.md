...
If `extensions/{client_slug}_extension/manifest.json` does not exist, prepare it from the local extension template. Prefer the repo helper when available:
...
If the helper is not available, copy `solo-agency-collector/chrome-extension/` to `extensions/{client_slug}_extension/`, patch that client's `manifest.json` so the extension name starts with the client name, and create `client_binding.json`.

**Two-gesture install.** On a LOCAL runtime, trigger it yourself: call `POST http://127.0.0.1:17321/api/ui/{client_slug}/install-extension` — the bridge reveals the client folder highlighted inside extensions/ (macOS `open -R`, Windows `explorer /select`) AND opens the extensions page in the Chrome window already running, brought to the front, in the same action (it can shell out directly since it runs on the human's own machine: `open -a "Google Chrome" "chrome://extensions/"` on macOS, `cmd /c start "" chrome "chrome://extensions/"` on Windows, `google-chrome`/`xdg-open` fallback on Linux). Then poll `GET /status` until `extension_health.status` is recent (75-second grace window) after the human does the two clicks only the browser can ask of a person: turn on Developer mode, drag the highlighted folder onto the extensions page. The page also plays a short recorded video for the chosen browser (Chrome or Edge) right under those two steps — mention it to the human ("the page has a 30-second video showing exactly this" — in the human's own language); per the Show the install video rule, always print the two GitHub fallback links too, no matter what else worked: `https://github.com/soloagency/solo-agency/blob/main/solo-agency-collector/bridge-go/assets/setup_extension_chrome_small.mp4` (Chrome) and `https://github.com/soloagency/solo-agency/blob/main/solo-agency-collector/bridge-go/assets/setup_extension_edge_small.mp4` (Edge).

**Which browser.** Setup never asks which Chrome profile the human uses — most humans do not know what a profile is, and the extension is bound to the client by `client_binding.json`, never by a profile. Before that call, `GET http://127.0.0.1:17321/api/ui/{client_slug}/browsers` lists every installed Chromium-based browser, so the agent knows whether there is a real choice to ask about. The only browser question, asked at most once per client, is "Facebook của anh/chị đang đăng nhập ở Chrome hay Edge?" — and only when both are installed; with just one supported browser present, pick it silently. Chrome/Edge are supported, Brave/Vivaldi/Opera/Chromium are best-effort (work, untested), Safari/Firefox are not supported (tell the human to install Chrome instead) — per the full situational table in `playbooks/SETUP_FLOW_ENTRYPOINT.md`, "Kết nối Facebook, Instagram and X (step 4)". The install page itself shows its own account dropdown only when the chosen browser has 2+ profiles — the human picks the one signed into Facebook there, and the page, not the agent, records the result into `extension_registry.json` (`browser`, `profile_directory`); once resolved, nothing about the browser or that account is ever asked again for the same client.

**If check-in stalls — 90-second help loop.** If `extension_health` has not gone recent within 90 seconds of triggering the install action, do not just repeat the reminder. Diagnose in this order and say ONE short line naming the likely cause: (1) Developer mode is still off — the toggle top-right on the extensions page; (2) the folder was dropped into a different browser profile/window than the one logged into Facebook; (3) the browser was closed or the page was navigated away. Re-trigger the install action (`POST /api/ui/{client_slug}/install-extension`, with the same remembered `{browser, profile_directory}`) so Finder and the extensions page are back in front, then wait again. Run this diagnose-and-retrigger cycle at most 3 rounds. If the human says they cannot find the Developer mode toggle, that is a managed (work) Chrome — say so plainly, ask for a personal computer or a personal Chrome profile, and only then offer the web-only escape (`playbooks/SETUP_FLOW_ENTRYPOINT.md`, "Kết nối Facebook, Instagram and X (step 4)"). Never offer that escape before this help has been given, and never in the first message.
...
After this one-time setup, keep that Chrome window logged in to the private data sources approved for {Client Name}. The shared Local Collector app/bridge handles routing and local file output.
```
