...
**Two-gesture install.** On a LOCAL runtime, trigger it yourself: call `POST http://127.0.0.1:17321/api/ui/{client_slug}/install-extension` — the bridge reveals the client folder highlighted inside extensions/ (macOS `open -R`, Windows `explorer /select`) AND opens the extensions page in a new window of the chosen profile, brought to the front, in the same action (it can shell out directly since it runs on the human's own machine: with no explicit browser chosen, `open -a "Google Chrome" "chrome://extensions/"` on macOS, `cmd /c start "" chrome "chrome://extensions/"` on Windows, `google-chrome`/`xdg-open` fallback on Linux). Then poll `GET /status` until `extension_health.status` is recent (75-second grace window) after the human does the two clicks only the browser can ask of a person: turn on Developer mode, drag the highlighted folder onto the extensions page. The page also plays a short recorded video for the chosen browser (Chrome or Edge) right under those two steps — mention it to the human ("the page has a 30-second video showing exactly this" — in the human's own language).

**Which browser, which profile.** Before that call, `GET http://127.0.0.1:17321/api/ui/{client_slug}/browsers` lists every installed Chromium-based browser and its profiles (display name, signed-in email), so the agent can pass optional `{browser, profile_directory}` fields on `install-extension` to open the extensions page in the exact right window. Ask at most ONE browser question and ONE profile question per client — Chrome/Edge are supported, Brave/Vivaldi/Opera/Chromium are best-effort (work, untested), Safari/Firefox are not supported (tell the human to install Chrome instead) — per the full situational table in `playbooks/SETUP_FLOW_ENTRYPOINT.md`, "Kết nối Facebook, Instagram and X (step 4)". Once resolved, the choice is remembered per client in `extension_registry.json` (`browser`, `profile_directory`) and never re-asked.
...
**Easiest — one button:** open `http://127.0.0.1:17321/ui/{client_slug}/extension` and click the button. It reveals the client folder highlighted inside extensions/ (macOS `open -R`, Windows `explorer /select`), opens the extensions page in a new window of the chosen profile and brings it to the front, in the same click. Turn on `Developer mode`, then drag the highlighted folder onto the extensions page. It turns green ✓ when it connects. Right under those two steps, the page also plays a short recorded video for your browser (Chrome or Edge) showing exactly this.

**Manual fallback:**
1. Open the browser + profile already logged into Facebook for {Client Name}. Chrome or Edge is supported; Brave/Vivaldi/Opera/Chromium works but is untested; if only Safari or Firefox is installed, install Chrome first — it's free, and using it just for Facebook is fine.
...
After this one-time setup, keep that browser profile logged in to the private data sources approved for {Client Name}. The shared Local Collector app/bridge handles routing and local file output.
```
