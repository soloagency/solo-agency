...
function removeChildTags(parent, tagNames) {
...
function processNode(node, except_tags,baseLocation,prefix_text) {
  if(prefix_text){
    ...
    if(node){
        if (node.nodeType === Node.TEXT_NODE) {
          if (node.parentNode && node.parentNode.tagName === 'IMG') {
            ...
            if(last_Text != node.nodeValue) {
                combinedText.push(prefix_text+" "+node.nodeValue);
                last_Text = node.nodeValue;
            ...
            if (node.tagName === 'A') {
                ...
                if (!node.href.startsWith('http://') && !node.href.startsWith('https://')) {
                    const urlObj = new URL(node.href, baseLocation);
                    node.href = urlObj.toString();  // Now node.href is an absolute URL
                }
                ...
                var scripts = node.getElementsByTagName('noscript');
                if (scripts.length > 0) {
                    // Skip processing this node
                    return;
                }
                ...
                // Skip if the link is 'javascript:void(0)'
                if(linkHref === 'javascript:void(0)') {
                    return;
                }
                if(last_Text != linkText) {
                    combinedText.push(linkText + ': ' + linkHref + ' ');
                    last_Text = linkText;
                    lastElementType = "text";
                    allURLs.push(linkHref)
                }

                if (linkHref.endsWith('.pdf')) {
                    // downloadPDF2(linkHref);
                }
            } else if (node.tagName === 'IMG') {
              if(imageURLs.indexOf(node.src)==-1){
                imageURLs.push(node.src); // Capture the image source URL
                lastElementType = "image";}
            } else if (node.tagName === 'LI' || node.tagName === 'P' || node.tagName === 'DIV' || node.tagName === 'BR' || node.tagName.match(/^H[1-6]$/) != null) {
                // Append a newline character when certain tags are encountered
                var childNodes = node.childNodes;
                for (var i = 0; i < childNodes.length; i++) {
                    processNode(childNodes[i], except_tags, baseLocation);
                }
                combinedText.push('\n');
                lastElementType = "text";
            } else {
...
function isProductPage(textContent) {
    ...
    const isProduct = productKeywords.some(keyword => {
        ...
        if (occurrences > 0 && occurrences <= 2) {
            matchedKeywords.push(keyword); // add the matched keyword to the array
            return true;
        }
        return false;
    });
...
function checkMoneyNearButtons(html, buttonTitles) {
  // Parse the HTML
  ...
  // Check if money appears near buttons
  let isMoneyNearButton = false;
  for (const moneyEl of moneyElements) {
    for (const buttonEl of buttonElements) {
      const moneyText = moneyEl.textContent.trim();
      const buttonText = buttonEl.textContent.trim();
      // Check if they are in the same parent
      if (moneyEl.parentElement === buttonEl.parentElement) {
        console.log(`Money (${moneyText}) appears near the "button" (${buttonText})`);
        isMoneyNearButton = true;
      }
...
function remove_relate_items(text) {
  var relate_texts = ["Parts & Accessories","You might also like","YOU MAY ALSO LIKE", "FREQUENTLY BOUGHT WITH", "Frequently bought together", "Related to items", "Related products", "Customers also viewed", "People also searched for", "Recommended for you", "Related to items you've viewed", "Suggested items", "Similar items", "You might also be interested in","Deals on related products","Compare with similar items","Products related to this item","Brands in this category on Amazon"];
  var regex = new RegExp(relate_texts.join('|'), 'i');
  var splitText = text.split(regex);
  return splitText[0]; // return the part of the text before the first matching related text
}
...
function cleanHTML(htmlString) {
  ...
  // 1. Remove all link tags and their content
  const anchors = doc.querySelectorAll('a');
  anchors.forEach(anchor => anchor.remove());

  // 2. Extract and return plain text
...
function compareUrls(url1, url2) {
  try {
    ...
    // Normalize the host (e.g., remove 'www.')
    const host1 = parsedUrl1.hostname.replace(/^www\./, '');
    const host2 = parsedUrl2.hostname.replace(/^www\./, '');
    ...
    // Normalize the pathname (e.g., remove trailing slash)
    const pathname1 = parsedUrl1.pathname.replace(/\/$/, '');
    const pathname2 = parsedUrl2.pathname.replace(/\/$/, '');
    ...
    // Normalize the search string (query parameters)
    const search1 = parsedUrl1.search === '?' ? '' : parsedUrl1.search;
    const search2 = parsedUrl2.search === '?' ? '' : parsedUrl2.search;

    // Check if the search strings are the same
    if (search1 !== search2) {
      return false;
    }
...
function extractElements(text, offset = 0) {
...
function mergeAndRemoveDuplicateElementsOrdered(block1, block2) {
  ...
  for (const element of allElements) {
    if (element.type === 'sentence') {
      ...
      if (!seenSentences.has(trimmedSentence)) {
...
function injectScriptToCallFunction(param1, param2, param3, param4) {
  const script = document.createElement('script');
  script.textContent = `
    send_message("${param1}", "${param2}", ${param3}, ${param4});
  `;
  (document.head || document.documentElement).appendChild(script);
  script.remove();
}

function convertRelativeUrlsToAbsolute(html, baseLocation) {
  // Create a temporary DOM div element
  const tempDiv = document.createElement('div');
  tempDiv.innerHTML = html;

  // Get all anchor tags within the temporary div
  const anchorTags = tempDiv.querySelectorAll('a');

  // Iterate over each anchor tag
  anchorTags.forEach(anchor => {
    // Get the href attribute using getAttribute (to get the exact value from HTML)
    const href = anchor.getAttribute('href');

    // Only process if href is not null, and is a relative URL
...
function getMainImageUrl(doc) {
    ...
    // Check for generic image meta tag
    const genericImage = doc.querySelector('meta[name="image"]');
    if (genericImage && genericImage.content) {
        return genericImage.content;
    }

    // Return null if no image meta tags were found
    return null;
}

function do_readability(message,simple_html, simple_title,url){
    const productKeywords = ['add to cart','out of stock', 'buy now', 'order now', 'add to wishlist', 'add to bag', 'reserve now', 'inquire now','in stock','sold out'];
   last_Text=""
   combinedText=[]
   lastElementType=""
   allURLs=[]
   imageURLs=[]
  simple_html=cleanStyle(simple_html)
  var loc = new URL(url);
  var uri = {
      spec: loc.href,
      host: loc.host,
  ...
  var item={}
  item.title=simple_title.replace(/"/g, '“').replace(/'/g, '’')

  const parser = new DOMParser();
  const BaseLocation=getBaseLocation(url)
  simple_html=convertRelativeUrlsToAbsolute(simple_html, BaseLocation);
  ...
  item_readability.combinedText=remove_relate_items(cleanText(getHumanReadableText(doc.documentElement.outerHTML,productKeywords)))
  console.log("JUST READ>>>",message.html)
  console.log("READABILITY>>>",doc.documentElement.outerHTML)
// console.log("===========",simple_html)
  item.combinedText=mergeAndRemoveDuplicateElementsOrdered(item_readability.combinedText,item.combinedText)

  item.combinedText=cleanText(item.combinedText.replace(/\\n+/g, ' ').replace(/\n+/g, ' ').replace(/"/g, '“').replace(/'/g, '’').trim())
