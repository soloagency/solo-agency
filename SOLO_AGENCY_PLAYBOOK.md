...
○ 5. I set up your sources: the default ones (Google search with your keywords, Facebook, Instagram, X, and the sites for your industry) plus any URL you give me to watch — groups and pages the runs find are added to your sources automatically (pause any on the Sources page)
...
- Step 6 is the one-time schedule/routine plus client-specific automation task setup. It should happen before step 7 (Chạy lượt đầu) so the automation task exists and the dispatch preconditions are satisfied first.
...
- Custom URLs (step 5) and automatic discovery were treated independently — a 'no' to custom URLs never turned monitoring off.
- Custom-source intake happened once, at step 5 — the human provided URLs, declined, or postponed. Joined-places discovery was never separately offered or declined; it rode the step-7 first-run answer, and its results were monitored automatically via the Group Potential Rule.
