...
7. **Chạy lượt đầu (First run).** Once the automation task from step 6 exists and its preconditions are met — `facebook_lead_source` (and `instagram_lead_source`/`x_lead_source` alongside it) is `enabled` or `web_only`, never `pending`, and on a local Claude Code runtime the step-6 unattended-permissions consent was asked — ask the Boss exactly one question, translated into the human's language, keeping the shape (Vietnamese sample):
   ...
   No source-review question comes before or after it — nothing else is asked. On yes: record `first_run_consent: yes`; verify/resync the client-specific automation task; then dispatch and wait exactly per `playbooks/04_DAILY_SCHEDULE.md` ("Run-now per runtime", "Wait and report") — `tool run-progress eta --calls <calls_planned>`, dispatch per runtime (Claude: one-time task `fireAt` now+2 min; Codex: run-now), the three dispatch sentences (time needed for the Boss's account safety; progress posted after each stage plus a message when it finishes; the WideCast PDNA Notification/Telegram line), and arm `tools/wait_for_run ... --watch progress`. Joined-places discovery (`private_data_source_discovery`, Stage 2) rides the same yes with no separate question: record `approved_pending_first_scan` for all its categories, and this run executes it — the results go through the Group Potential Rule straight into monitored sources, never a shortlist, never a separate approval. On "để sau" / not now: record `first_run_consent: not_now`; the daily task keeps its existing schedule; mark roadmap step 7 `–` with the reason "runs at {HH:MM} on the schedule"; ask nothing else.
