...
**Client:** {Client Name}
**Load it into:** the Chrome window you normally use for Facebook (Edge if that is where you are logged in)
...
**Video:** {the `/ui/{client_slug}/extension` page when the bridge is up, else the GitHub video link}
**Then reply:** `done`
**Why:** This folder carries the client binding; any window logged into the right Facebook account works.
```

See "Show the install video" in `playbooks/SETUP_FLOW_ENTRYPOINT.md` for the full three-way rule behind the `**Video:**` line above.
