...
**Fifth fixed line: the CRM link.** When any scan since the Boss was last here (daily monitoring, the search pass, or the Facebook Discovery Pass) captured ≥ 1 lead, the frame ends with the CRM link line from `playbooks/10_LEAD_COMPETITOR_DETECTION.md` ("Show the CRM link after any scan that produced ≥ 1 lead"):
...
**Seventh fixed line, when it applies: web-only awareness.** While this client's `facebook_lead_source` is `web_only` (the acknowledged, explicit choice recorded at setup step 4 or the tier-2 `fb_web_only_upsell` offer — never a default, never "để sau"), the frame carries one more line, in the human's language, per `playbooks/06_AGENCY_REPORT_STANDARD.md`:
...
`{date}` is `facebook_lead_source_updated_at`. It stops appearing once Facebook is enabled, and it never appears in a client-facing artifact.
...
- The public keyword bank includes pain-point/problem/need keywords, not only generic industry keywords, uses the target audience's search language, and the full bank was staged in `public_keywords_seed.jsonl` for `tool public-keywords add --file` at the first run.
...
- The step 7 private data source intake/discovery/approval was resolved, declined, postponed, or honestly marked pending, and the automation task was resynced or confirmed current afterward.
...
- If private data sources were requested or the human was unsure, step 7 offered manual source intake and optional private data source discovery from approved joined groups, subreddits, communities, followed profiles/pages/KOLs, subscribed channels, and feeds, or recorded that the human declined/postponed it.
...
- If an automation agent is about to run the first report before private data source status, the step 7 private data source checkpoint, and schedule/routine are resolved or honestly marked pending, stop and load the needed stage.
...
- Did I avoid jumping past schedule/routine setup, client-specific automation readiness, the step 7 private data source/Local Collector checkpoint, approval gates, or measurement gates?
